# Capítulo 15 — El clímax: La Torre de la Eternidad

![Corte vertical de la Torre de la Eternidad](assets/mapas/12_torre-eternidad.png)

> **El final de la campaña** · **Nivel:** 10 (techo de Chronosia) · **Reloj:** 8/8 · **Puerta:** se abre cuando los dioses claman · **Lo que se decide:** si Aethernus asciende, si Teach lo usurpa, o si los héroes liberan a Amaunator y Voidar y devuelven el equilibrio.

Todo lo que los héroes han hecho —cada región explorada, cada lugarteniente caído, cada facción reconciliada— converge aquí, en una sola torre suspendida entre los planos. La Torre de la Eternidad no es una mazmorra: es la última página de Chronosia, y los PJ la escriben con sus elecciones. Este capítulo cuenta el clímax como debe vivirse en mesa: la llamada, el asalto nivel a nivel, la batalla por fases y los cuatro finales posibles.

---

## La Llamada de los Dioses

Cuando el **Reloj del Ritual llega a 8/8**, el ritual de ascensión entra en su última fracción y el semiplano entero lo siente. No hay heraldos ni mensajeros: los propios dioses cautivos, exprimidos hasta el límite, encuentran el aliento para gritar una vez. Léelo cuando el último segmento del Reloj caiga en su sitio.

> *Leído a todos los PJ, estén donde estén:*
> "El cielo de Chronosia se raja como una vidriera vieja. Por las grietas no se ve cielo: se ve **el cosmos colapsando**, galaxias que se derraman hacia adentro. El tiempo tartamudea —vuestro corazón late dos veces y luego se salta un latido— y el espacio se pliega: el horizonte que tenéis delante se acerca y se aleja como si respirara.
>
> Y entonces, dentro de vuestra cabeza, **dos voces** que no son una sola. La primera es luz, y suena a mediodía y a derrota:
>
> «**Mortales. Habéis llegado.** El ritual está casi completo. Si no actuáis *ahora*, el ladrón será invencible. Yo soy el sol que mide vuestros días, y me han contado hasta el último. La Torre se ha vuelto visible. **Venid.**»
>
> La segunda es frío, y suena a vacío entre estrellas:
>
> «**El pirata también viene.** Edward Teach tiene el Talismán. Quiere lo que el ladrón roba, para sí. Si él llega primero… o si vosotros llegáis tarde… el multiverso tendrá un dios nuevo, y será peor. **Corred.**»
>
> Cuando las voces callan, en el centro fracturado del semiplano —donde nunca hubo nada— se alza una **torre**. Estaba ahí desde siempre, escondida en un doblez del espacio; ahora el doblez se ha deshecho. De su cima brotan dos columnas de luz que perforan el cielo roto: una **dorada**, otra **púrpura**, trenzándose en un vórtice que late como un corazón a punto de pararse."

**Los aliados se reúnen.** La Llamada la oye *todo* Chronosia. En las horas que siguen, lo que los PJ hayan construido durante el sandbox se cobra su valor. No improvises ayuda que no se ganaron; reparte exactamente lo que merecen:

- **Anacronistas (Cronópolis).** Si los PJ se ganaron su confianza, Din Goldgear les entrega lo que queda del arsenal gnomo (pociones, un par de artefactos menores) y, sobre todo, **su mitad de la llave chroniana** y un puñado de portadores del saber temporal.
- **Oceánicos de Abysara.** Si los PJ los reconciliaron, suben de las profundidades con su mitad del saber —el dominio del **espacio**— y guerreros leales hasta la muerte. *Sin ambos pueblos juntos, no hay llave. Recuérdalo: es la única vía para salvar a los dioses, y se decide aquí por lo que se hizo antes.*
- **Facciones menores.** La Resistencia, neutrales, supervivientes de las regiones: bendiciones, mapas de la Torre, suministros. Sabor y moral, no la victoria.

**Teach parte el primero.** Si Edward Teach sigue vivo, los PJ lo ven lanzarse hacia la Torre antes que nadie, riendo, con el Talismán latiéndole en el pecho.

> *Leído (Teach, a lo lejos, sobre la cubierta de un barco que ya no toca el agua):*
> "¡Una **carrera**, héroes! El primero en llegar a la cima se queda con el poder de un dios. ¡Que gane el mejor… o sea, yo!"

Y entonces los héroes echan a correr hacia una torre que existe en varias dimensiones a la vez, con el Reloj sonando 8 y el ritual rozando lo irreversible.

---

## El asalto, nivel a nivel

La Torre se sube de abajo arriba, y el mapa lo dice todo: cuatro niveles encajados como las cámaras de un reloj de arena. Entre nivel y nivel hay **antesalas** donde todos los que entraron —PJ, Teach, lugartenientes vivos— se ven y se oyen pero **no pueden atacarse**, y donde el **medidor del ritual** cuelga del aire, visible, subiendo. La antesala es el respiro y la amenaza: nadie puede tocar a nadie, pero todos ven el porcentaje trepar, y nadie ha olvidado lo que viene arriba.

### Nivel 1 · Sala de Ecos — *la revelación masiva*

En el plano del mapa, la base de la Torre: una cámara circular forrada de **espejos y marcos** de todos los tamaños, inclinados, cruzados, reflejándose entre sí hasta el infinito. Aquí no se combate: aquí se *comprende*. Los espejos no devuelven el reflejo de los PJ; devuelven la **verdad**.

> *Leído al entrar:*
> "La sala es un bosque de espejos. Decenas, cientos, en marcos dorados y carcomidos, apoyados unos contra otros, colgando torcidos del techo. Esperáis ver vuestra cara. No la veis. En cada cristal pasa **otra cosa**: en uno, un anciano de coleta plateada inclinado sobre dos figuras encadenadas. En otro, ese mismo anciano más joven, vistiendo las ropas de un hechicero de Faerûn, repitiendo un nombre que conocéis: *Manshoon*. En un tercero, un pirata robándole un talismán a una mujer dormida que no vuelve a despertar. Los espejos no mienten. **Lo cuentan todo a la vez.**"

**Qué ocurre:** la sala resuelve, de golpe y para todos los presentes, lo que el sandbox repartió en migas por las regiones. Recorrer los espejos (o resolver los acertijos que los desbloquean) destapa la **verdad completa**:

- **Aethernus Valcarys es Manshoon, el Clon #47.** El archimago de Faerûn que sembró el multiverso de copias de sí mismo. Una de ellas encontró Chronosia.
- **Hay dos dioses presos:** Amaunator (Tiempo) y Voidar (Espacio), encadenados arriba, exprimidos gota a gota.
- **Los lugartenientes nunca tuvieron poder propio:** eran conductos. Lo que drenaban era luz robada al sol y vacío robado al vacío.
- **El plan completo de ascensión** y que **el Talismán de Teach puede interceptar el poder divino.**
- **La guerra civil entre lugartenientes fue espontánea**, no un plan: Aethernus, recluido, ni se enteró.

> *Si los PJ ya habían deducido casi todo (sandbox completo), juega los espejos como confirmación devastadora —"así que era verdad". Si llegan medio ciegos (corrieron demasiado), es aquí donde se les cae el mundo encima. En ambos casos, **Teach y los lugartenientes vivos lo oyen también**: a partir de este nivel, todos saben lo mismo.*

Resueltos los espejos, una escalera de caracol asciende. Las antesalas separan cada tramo; el medidor sube.

### Nivel 2 · Corazón del Tiempo — *la mansión-trampa de clones*

El mapa lo muestra partido en dos mitades que no deberían convivir: a un lado, los **salones lujosos** de una mansión —alfombras, candelabros, retratos—; al otro, una pared de **engranajes de reloj** girando, gigantescos. Era la casa donde Manshoon vivió antes de encerrarse en el ritual. El tiempo aquí es dulce, y por eso es una trampa.

> *Leído al entrar:*
> "La escalera os deja en un recibidor que huele a cera y a vino caliente. Hay fuego en la chimenea, sillones que parecen recordar vuestra forma, una mesa puesta. Por una pared corren engranajes del tamaño de ruedas de molino, marcando un tiempo que aquí dentro va… *despacio*. Y en cada habitación, esperándoos, hay **alguien**. Alguien con vuestra cara. Vuestra voz. Vuestros recuerdos. Sonríe, y os dice que ya podéis descansar."

**Qué ocurre:** cada habitación guarda un **tesoro** (una poción, un objeto mágico —recursos reales para la batalla de arriba) y un **clon distorsionado de uno de los PJ**, que no ataca: *tienta*. Le ofrece al héroe exactamente lo que más desea y nunca tuvo. "Aquí no envejeces. Aquí nadie a quien quieres ha muerto. Aquí ya has ganado. Quédate."

- No hay tirada que lo resuelva sola: es **interpretación**. El jugador decide si su personaje cede o se arranca de la fantasía. Pídele que diga en voz alta qué le ofrecen y por qué se va igualmente —ese es el peso.
- El que cede *se queda* (queda fuera del clímax, atrapado en su día perfecto) salvo que un compañero lo saque a rastras. El que resiste se lleva el tesoro de su habitación.

Es el **espejo amable** del villano: Manshoon también se quedó en su mansión, en su sueño de poder, mientras el mundo ardía afuera. Quien atraviesa el Corazón del Tiempo sin quedarse ha entendido —con el cuerpo, no con la cabeza— de qué va todo esto.

### Nivel 3 · Eliminatoria — *la prueba*

El mapa lo dibuja como un **puente de piedra en espiral** que cruza un abismo del que suben tentáculos de sombra. No hay enemigos que matar: hay un vacío que cruzar con la sola fuerza de la voluntad. Es el filtro de la Torre: no todos los que empiezan el asalto llegan a la cima.

> *Leído al entrar:*
> "Ante vosotros, un puente de piedra se enrosca sobre un abismo sin fondo. Del vacío suben zarcillos de oscuridad que no tocan, solo *susurran*: os dicen vuestro peor recuerdo, vuestra peor culpa, la frase que más os duele. Cruzar no es cuestión de pies. Es cuestión de no escuchar."

**Qué ocurre:** cada criatura que cruza hace una tirada de **salvación de Sabiduría CD 18**. Quien falla **no puede avanzar al Nivel 4** —el puente lo rechaza, lo deja atrás (no lo mata; lo aparta). Quien la supera, llega a la Cámara de Ascensión.

- **Edward Teach cruza con ventaja:** el Talismán lo blinda contra la prueba. Si vive, casi siempre llega arriba. Esto *importa* para el Final B.
- Es el momento de la verdad para los lugartenientes vivos y los aliados frágiles: algunos se quedan en el puente. Quien llegue a la cima es quien pelea el final.

### Nivel 4 · Cámara de Ascensión — *los dos dioses encadenados + Aethernus*

La cima. El mapa la corona con todo lo que la campaña prometía: un **círculo ritual zodiacal** grabado en el suelo, dos **pilares** a los lados, y entre ellos, lo impensable. A la izquierda, una figura de **oro**, radiante y vencida, encadenada por cadenas de luz: **Amaunator**, el sol prisionero. A la derecha, una figura de **púrpura** envuelta en velos de vacío, encadenada igual: **Voidar**, el espacio prisionero. Y en el centro exacto, flotando entre ambos, una semilla de luz que es a la vez **un sol diminuto y una estrella negra** —el eclipse donde tiempo y espacio se drenan el uno en el otro. Ese es el **Cronosellado**, el núcleo del ritual. Sobre él, de espaldas, con los brazos abiertos, **Aethernus Valcarys**.

> *Leído al entrar:*
> "La escalera muere en una plataforma abierta al cosmos roto. Dos columnas la flanquean. Encadenado a la de la izquierda, un dios de oro tan brillante que duele mirarlo y tan agotado que apenas se sostiene: cada cadena que lo sujeta **gotea luz**, y la luz cae hacia el centro. Encadenado a la derecha, una figura de sombra violeta entre la que se ven estrellas, igual de presa, igual de exprimida. Y en medio de los dos, donde sus poderes robados se cruzan, late un punto imposible —un sol y un agujero a la vez— rodeado de un círculo de signos grabado en el suelo.
>
> De espaldas, sobre ese punto, hay un anciano de coleta plateada y túnica negra bordada en oro. No se vuelve. Habla con el cosmos, no con vosotros:
>
> «Casi. *Casi.* Ochocientos años escondidos, dos dioses en mi puño, y solo me falta el último latido. Habéis subido muy lejos, pequeños. Os felicito. Pero llegáis para ver un nacimiento, no para impedirlo.»
>
> Entonces sí se vuelve. Y veis que el medidor del ritual, flotando sobre el Cronosellado, marca un número que sube solo."

A partir de aquí, todo es la batalla final. Si Teach llegó, está presente, con el Talismán encendido, esperando su momento. Si quedan lugartenientes vivos que cruzaron, eligen bando. Y los dos dioses, todavía encadenados, miran a los héroes con la última esperanza que les queda.

---

## La batalla final por fases

> **Nivel 10 fijo** (no se sube de nivel durante el combate). **Reloj 8/8.** El **medidor del ritual** está visible siempre y sube cada 2-3 turnos (narrativo, no estricto). Si llega al 100% antes de que los PJ ganen → **Final D**. La estructura es de **tres pasos** (interrumpir → liberar → derrotar), entrelazados con las fases del statblock de Aethernus.

### Paso 1 — Interrumpir el ritual *(detener el reloj)*

Mientras el Cronosellado lata, Aethernus se regenera y el medidor sube. Hay que **callar el núcleo** antes que nada.

- **Acción:** atacar el **Cronosellado** (CA 20) o, como acción, **salvación de Sabiduría CD 18** para imponerle el propio espíritu.
- **Efecto:** el ritual se **detiene temporalmente** y el medidor retrocede 1-2%. Aethernus pierde su regeneración de fase (entra en **Fase 2** de su statblock). Sin esto, los dioses no pueden liberarse y el medidor sigue trepando.

### Paso 2 — Liberar a los dioses *= SOLO la llave chroniana*

Esta es la decisión que se tomó hace muchas sesiones, no aquí. Las cadenas de Amaunator y Voidar están hechas de **tiempo y espacio combinados**: el saber que sabía contenerlas —y por tanto soltarlas con cuidado— murió cuando los Chronianos se partieron en dos. Recomponerlo es **la llave chroniana**.

> **⚠️ ÚNICA VÍA.** Solo se liberan los dioses si los PJ **reunieron a los dos pueblos** durante el sandbox: **Anacronistas** (saber del Tiempo) **+ Oceánicos de Abysara** (saber del Espacio). Juntos, portadores de ambas mitades ejecutan el método chroniano sobre las cadenas: el lado temporal sujeta a Amaunator mientras el dimensional sujeta a Voidar, y por primera vez en ochocientos años las dos fuerzas dejan de drenarse. **No hay rutas alternativas.** Nada de persuadir a los dioses con CD 25; nada de combinar artefactos sueltos. Si no se reunieron las facciones, **las cadenas no se sueltan** —los héroes pueden seguir y matar a Aethernus, pero los dioses no se salvan (desenlace agridulce, ver finales y epílogo).

- **Con la llave:** las cadenas se abren, los dioses **cesan su lucha eterna** y vuelven sus rostros —uno de oro, otro de sombra— hacia Aethernus. A partir de aquí, **luchan con los PJ.**
- **Sin la llave:** los héroes afrontan la fase final solos, y el destino de los dioses pende del modo en que caiga Aethernus.

Es el pago de todo el tema de la campaña: *los pueblos partidos se necesitan para sanar*. La reconciliación que parecía diplomacia menor en el sandbox es, literalmente, lo que decide si dos dioses viven.

### Paso 3 — Derrotar a Aethernus

Con el núcleo interrumpido y (idealmente) los dioses sueltos, queda vencer al archimago antes de que el medidor toque el 100%. Aethernus pelea **por fases**: tres barras de vida consecutivas; al agotar una, transiciona a la siguiente con un nuevo monólogo. Sus poderes dependen de **qué lugartenientes seguían vivos** al empezar: por cada lugarteniente que el grupo ya derrotó en el sandbox, **tacha** un poder robado de su Fase 2. El trabajo previo del grupo se cobra aquí, mecánicamente.

Si los dioses fueron liberados, intervienen entre fases: **Amaunator** lanza envejecimiento y bucles temporales, **Voidar** comprime y fragmenta el espacio bajo los pies de Aethernus. No ganan el combate por los PJ —lo hacen **ganable**.

> *Leído al caer la última barra (con la llave):*
> "Aethernus se desploma sobre el círculo apagado. El sol diminuto y la estrella negra del Cronosellado parpadean… y se separan, por fin, en una luz dorada y una sombra violeta que ya no se drenan. El anciano levanta la vista hacia los dos dioses libres, y por primera vez en ochocientos años entiende lo que es ser pequeño. «…No estaba escrito», susurra. Y se apaga."

---

### Statblock compacto — Aethernus Valcarys (Manshoon Clon #47)

*Humanoide mediano (humano), legal maligno* · **CR 13-14** (11.500 XP) · *(referencia completa: 02_Guia_DM/04_Cronofagos_Detallado/12_Aethernus_Valcarys_Manshoon.md)*

![Aethernus Valcarys](assets/bestiario/lugartenientes/12_aethernus-valcarys.jpg)

**CA** 19 · **PG** 300 en **3 barras de fase** (110 / 100 / 90) · **Vel.** 30 pies, vuelo 60 pies
**FUE** 18 (+4) · **DES** 16 (+3) · **CON** 16 (+3) · **INT** 22 (+6) · **SAB** 18 (+4) · **CAR** 20 (+5)
**Salv.** DES +8, CON +8, INT +11, SAB +9, CAR +10 · **Resist.** temporal, espacial; daño no mágico
**Inmune (estado):** encantado, agotado, envenenado, asustado · **V. verdadera** 120 pies · **P. pasiva** 19 · **Idiomas:** todos

- ***Combate por fases.*** No es un saco de 300 PG: al vaciar una barra, transiciona (gratis, sin AdO), recupera el repertorio de la nueva barra y suelta un monólogo. Muere solo al vaciar la barra de Fase 3.
- ***Resistencia Legendaria (3/día).*** Convierte un fallo de salvación en éxito. **No se recarga** entre fases.
- ***Inmunidad a Predicción.*** Inmune a *presagio* y efectos que adivinen sus acciones.
- ***Acciones Legendarias (3/ronda):*** *Ataque de Espada* (1) · *Paso Dimensional* — teletransporte 60 pies (1) · *Estallido* — un enemigo a 60 pies, CD 18 (2).

**🟢 Fase 1 — El Estratega Recluido (110 PG).** *Regeneración:* +10 PG al inicio de su turno (desaparece en Fase 2).
- ***Multiataque:*** dos *Espadas de Poder Divino* (+11, 2d10+5 cortante + 3d6 temporal/espacial; CD 18 CON o envejece 1d6 años / teleport 30 pies).
- ***Manipulación Temporal (3/día):*** radio 60 pies, CD 18 SAB o *lentitud* (enemigos) / *celeridad* (aliados).

**🟡 Fase 2 — El Cónclave Encarnado (100 PG).** Pierde la regeneración; gana los poderes de los **lugartenientes que sigan vivos** (tacha los caídos).
- ***Multiataque:*** tres ataques (dos *Espada* + uno *Bastón de Mando*: +11, 2d6+5 cont. + 3d6 mágico, CD 18 SAB o encantado 1 turno).
- ***Llamada a Lugartenientes (1/combate):*** hasta 2 lugartenientes vivos aparecen y luchan por él.
- ***Extracción de Poder (rec. 5-6):*** enemigo a 30 pies, CD 18 CON o desventaja al ataque; Aethernus cura 25 PG.
- ***Poderes robados (solo de lugartenientes VIVOS):*** p. ej. envejecimiento 1d4 años (CD 18 CON); *Portal de Fuego* cono 60 pies, CD 18 DES, 12d6 fuego. *Cada lugarteniente caído antes = un poder menos.*

**🔴 Fase 3 — La Ascensión Truncada (90 PG).** Absorbe a cualquier lugarteniente invocado (cura 20 PG).
- ***Inestabilidad del Ritual:*** al inicio de su turno, la criatura no aliada más cercana hace CD 18 DES o 2d6 temporal/espacial.
- ***Multiataque:*** dos *Espada* potenciada (daño temporal/espacial sube a 4d6).
- ***Combinación de Poderes (rec. 5-6):*** campo de 60 pies, CD 18 CON, 10d6 temporal/espacial (mitad si pasa) + aturdido 1 turno con fallo.

> **Al morir:** el ritual se interrumpe sin explosión; el poder acumulado se disipa. Si los dioses fueron liberados con la llave, cierran las heridas de Chronosia; si no, el plano queda dañado pero estable. Las distorsiones se revierten en 1d6 días.

---

## Los cuatro finales

El medidor del ritual, lo que los PJ hicieron en el sandbox y la suerte de Teach deciden cuál de estos se juega.

### Final A · Manshoon Ascendente *(la vía por defecto)*

Los héroes **interrumpen el ritual, llegan a tiempo y derrotan a Aethernus** en su tercera fase. Es el final "limpio": el archimago cae antes de tocar la divinidad. Lo que diferencia un Final A *luminoso* de uno *agridulce* es la **llave chroniana** —si reunieron a los dos pueblos, los dioses viven; si no, mueren al romperse el ritual aunque el villano caiga. Ver epílogo.

> *Leído:* "Ochocientos años de ambición se apagan en un suspiro de un anciano. El sol y el vacío que tenía encadenados parpadean, libres o moribundos según lo que trajisteis hasta aquí. La Torre empieza a temblar: ya no hay nadie que la sostenga entre los planos."

### Final B · Blackbeard el Usurpador *(si Teach llega con el Talismán)*

Si **Edward Teach sobrevivió, cruzó la Eliminatoria (con ventaja del Talismán) y llega a la Cámara**, tiene su propia carta: en el instante en que el ritual se interrumpe y el poder divino queda suelto, **lo intercepta para sí**. No quiere matar a Aethernus por justicia: quiere *quedarse con lo que Aethernus robaba.*

- Teach puede **ayudar a los PJ** a tumbar a Aethernus —y luego intentar **traicionarlos** en el último segundo, Talismán en alto, para drenar el poder él.
- Si lo consigue (porque los PJ lo dejaron, lo aliaron sin condiciones, o no pudieron pararlo), el villano cambia de cara: no asciende Manshoon, asciende **Barbanegra**. Un dios pirata, caprichoso y vanidoso, en vez de un archimago frío. *El multiverso no se salva: cambia de tirano.*
- Para evitarlo, los PJ deben **negarle el momento**: interrumpir su uso del Talismán, volverse contra él en cuanto Aethernus cae, o haber atado su lealtad antes (sandbox del Archipiélago).

> *Leído (si Teach usurpa):* "Teach alza el Talismán justo cuando el poder de los dioses queda suelto. «Lo siento, héroes —ríe—, pero un buen pirata no devuelve el botín.» La luz dorada y la sombra violeta no vuelven a sus dueños: se enroscan en el pirata, que crece, que ríe más fuerte, que ya no os mira como a iguales. **Habéis derrocado a un dios para coronar a otro.**"

### Final C · Carrera Divina *(el filo de la navaja)*

Cuando el medidor está **muy alto** (Teach corriendo, PJ rezagados, ritual cerca del 100%) el final se convierte en una **carrera literal contra el porcentaje**. Cada turno cuenta; interrumpir el Cronosellado retrocede el medidor, perder turnos lo dispara. Es el Final A o B comprimido en una cuenta atrás visible: los héroes ganan **por los pelos**, o lo ven completarse ante sus ojos (→ Final D). Úsalo para tensar al máximo cuando el grupo llegó tarde pero no *demasiado* tarde. La emoción está en el medidor subiendo mientras ruedan los dados.

### Final D · Demasiado Tarde *(imparable — fin de partida)*

Si el **medidor llega al 100%** antes de que los PJ liberen o derroten, **Aethernus consuma la ascensión**. No hay combate: ya es un dios, y un dios **no tiene CR ni statblock**. Es una **derrota narrativa**, no un encuentro ganable —resistir la tentación de inflarlo a CR 23/25 y "darles una oportunidad". No la hay. Esa es la honestidad del Reloj.

> *Leído:* "El medidor toca el cien. El sol y el vacío que estaban encadenados se… **vacían**, de golpe, como dos odres pinchados, y mueren. Lo que queda en pie en el centro del círculo ya no es un hombre. Os mira como un niño mira a una hormiga. «El multiverso es mío —dice, y su voz es todas las voces—. Vosotros sois insignificantes. Huid. Huid mientras el mundo que conocíais aún existe… porque pronto no existirá.» No hay tirada de ataque que importe. Solo queda **huir, o caer de pie.**"

Los PJ pueden **huir** (y vivir para contar el horror) o **sacrificarse heroicamente** (cubriendo la retirada de los demás, un último gesto en un mundo perdido). Cualquiera de las dos cierra la campaña. No la suavices: el peso del Final D es que el Reloj **siempre fue real**.

---

## Epílogo

### Con la llave chroniana *(el mejor desenlace)*

> *Leído:*
> "Aethernus yace inmóvil. El Cronosellado se ha apagado. Y por primera vez en ochocientos años, el sol y el vacío de Chronosia **no se hacen daño**.
>
> AMAUNATOR, recobrando la voz que es todos los amaneceres: «Habéis salvado el multiverso. Y habéis hecho algo que creíamos imposible: **reunir a los Chronianos**. Por eso podemos volver a contenernos. Por eso Chronosia vivirá.»
>
> VOIDAR, frío y sereno por fin: «El tiempo y el espacio vuelven a su sitio. Gracias, mortales. Recordad esto cuando dudéis de que algo roto pueda recomponerse.»"

Los dioses recobran su equilibrio y vuelven a contener su propia guerra; los portales dimensionales se cierran; las regiones fracturadas se restauran a lo largo de 1d6 días; los lugartenientes vivos pierden el poder robado y vuelven a ser lo que eran. Los Anacronistas y los Oceánicos —hermanos partidos durante ocho siglos— se miran a través de la grieta que se cierra y, despacio, vuelven a ser **un solo pueblo**. Los PJ regresan a Waterdeep como héroes legendarios, cargados de recompensas de las facciones y de algo que no se inventaría: la certeza de que un mundo entero existe porque ellos eligieron correr.

**El precio.** No todo se salva gratis. Recuerda los costes que el grupo dejó por el camino: los **Aldeanos Vueltos Niños** que solo vivían dentro del bucle, el **Hombre que Cuenta** que pidió que lo recordaran, el compañero que se quedó en el Corazón del Tiempo, los aliados que cayeron en la Eliminatoria. La libertad de Chronosia se pagó con ellos. Deja que los jugadores nombren a sus muertos. *Ese es el latido de la campaña: nada se salva gratis.*

### Sin la llave chroniana *(agridulce)*

> *Leído:*
> "Aethernus cae. El ritual se rompe. Pero las cadenas, hechas de un saber que nadie supo recomponer, se rompen mal: el sol dorado y la sombra violeta se apagan despacio, agradecidos y moribundos a la vez.
>
> AMAUNATOR, en un último susurro de luz: «Habéis vencido al ladrón… y aun así nos perdéis. No reunisteis a los nuestros. No os culpo. Pocos lo habrían hecho mejor. Recordadnos.» Y se apaga, como un mediodía que cae a plomo en noche."

Chronosia sobrevive —el villano cayó, las distorsiones se estabilizan— pero **sin sus dioses**: un plano huérfano, dañado, que tardará generaciones en sanar y quizá nunca del todo. Los PJ son héroes igualmente, pero cargan con la pregunta que no podrán contestar: *¿y si hubiéramos reconciliado a los dos pueblos cuando tuvimos la ocasión?* No lo conviertas en castigo; conviértelo en **memoria**. Es el otro rostro del mismo tema —el precio de no haber tendido un puente a tiempo.

### Si Barbanegra usurpó *(Final B)* o si fue Demasiado Tarde *(Final D)*

No hay epílogo luminoso, pero sí hay **cierre**. Cuenta qué fue del multiverso bajo el nuevo dios —el caos caprichoso de Barbanegra, o el dominio frío de Manshoon—, qué quedó de los aliados, y qué hicieron los PJ con el final: la huida que se vuelve resistencia, el sacrificio que se vuelve leyenda. Aunque pierdan, **fueron los que estuvieron allí cuando el Reloj sonó 8.** Que eso pese. Que se recuerde.

---

> *La Torre se deshace en el cielo fracturado, ladrillo a ladrillo, hasta que no queda más que el doblez de espacio del que salió. Lo que los héroes hicieron dentro de ella nunca se contará dos veces igual —porque cada partida llegó con sus propias piezas de la verdad, sus propios muertos y su propia llave. Esa fue siempre la promesa de Chronosia.*
