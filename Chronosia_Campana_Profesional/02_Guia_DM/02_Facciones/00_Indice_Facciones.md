# 🏛️ Facciones y Organizaciones
## *Los Poderes que Moldean Chronosia*

---

> **📖 NAVEGACIÓN:**
> Este es el índice principal de todas las facciones de Chronosia. Cada facción tiene su propio documento detallado.

---

## 📋 **ÍNDICE DE FACciones**

### **⚔️ LOS CRONÓFAGOS**
**Devoradores del Tiempo y el Espacio**

- **Líder**: Aethernus Valcarys (Manshoon Clone #47)
- **Actitud**: Hostiles hacia los PJ
- **Poder**: Manipulación temporal y espacial
- **📖 Documento completo:** [01_Cronofagos.md](./01_Cronofagos.md)

---

### **⚙️ LA RESISTENCIA**
**Cazadores de Magos**

- **Filosofía**: Odian la magia en todas sus formas
- **Actitud**: Neutrales, pueden ser aliados o enemigos
- **Especialidad**: Tecnología anti-magia
- **📖 Documento completo:** [02_La_Resistencia.md](./02_La_Resistencia.md)

---

### **🌿 LOS ANACRONISTAS**
**Guardianes del Tiempo Natural**

- **Concepto**: Resisten las distorsiones temporales
- **Actitud**: Amigables hacia los PJ
- **Rol**: Aliados clave y guías
- **📖 Documento completo:** [03_Anacronistas.md](./03_Anacronistas.md)

---

### **🗡️ BREGAN D'AERTHE**
**Mercenarios Contratados**

- **Líder**: Jarlaxle Baenre
- **Actitud**: Ambiguos, pueden ser aliados o enemigos
- **Función**: Espionaje y operaciones especiales
- **📖 Documento completo:** [04_Bregan_Daerthe.md](./04_Bregan_Daerthe.md)

---

### **🌊 LOS OCEÁNICOS DE LAS PROFUNDIDADES**
**Guardianes de Abysara**

- **Líder**: Reina Nereia
- **Ciudad**: Abysara (ciudad sumergida)
- **Actitud**: Aliados potenciales (después de la traición de Teach)
- **Especialidad**: Combate submarino, conocimiento de cristales
- **📖 Documento completo:** [07_Oceanicos_Abysara.md](./07_Oceanicos_Abysara.md)

---

### **⚔️ DINÁMICAS Y ESTRATEGIAS**
**Relaciones entre Facciones**

- **Dinámicas**: Conflictos y alianzas entre facciones
- **Estrategias**: Cómo aliarse con cada facción
- **Oportunidades Narrativas**: Hooks y giros argumentales
- **📖 Documento completo:** [06_Dinamicas_Entre_Facciones.md](./06_Dinamicas_Entre_Facciones.md)

---

## 🎯 **RESUMEN RÁPIDO**

| **Facción** | **Actitud hacia PJ** | **Especialidad** | **Aliado Potencial** |
|------------|----------------------|------------------|---------------------|
| **Cronófagos** | Hostiles | Manipulación temporal/espacial | ❌ No |
| **La Resistencia** | Neutrales | Tecnología anti-magia | ⚠️ Depende |
| **Anacronistas** | Amigables | Resistencia temporal | ✅ Sí |
| **Bregan D'aerthe** | Ambiguos | Espionaje y mercenariado | ⚠️ Depende |
| **Oceánicos de las Profundidades** | Aliados (después de traición de Teach) | Combate submarino, cristales | ✅ Sí |

---

## 📚 **REFERENCIAS RELACIONADAS**

- **Lugartenientes Detallados**: [../04_Cronofagos_Detallado/00_Cronofagos_Detallado.md](../04_Cronofagos_Detallado/00_Cronofagos_Detallado.md) (ver archivos individuales para statblocks completos)
- **NPCs Importantes**: [../03_NPCs_Importantes.md](../03_NPCs_Importantes.md)
- **Sistema de Balance**: [../../01_Como_Dirigir.md)

---

*Las facciones de Chronosia representan diferentes filosofías sobre el tiempo y el espacio. Elegir aliados sabiamente puede determinar el éxito o el fracaso de la misión.* 🎲✨

