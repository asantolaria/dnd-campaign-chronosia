# 🔱 El Talismán de Interceptación Divina
## *El Arma Secreta de Edward Teach*

---

> **⚠️ PROPÓSITO DE ESTE DOCUMENTO:**
> Este documento detalla el **Talismán de Interceptación Divina**, sus dos componentes (Cronómetro y Perla), los robos, y el asesinato de Thyra.
> 
> **Para cronología general de la campaña y otros eventos**, consulta: **[07_Cronologia_Maestra_Campana.md](../../02_Guia_DM/07_Cronologia_Maestra_Campana.md)**

---

## 📋 **ÍNDICE DE NAVEGACIÓN**

### **🎯 Información General**
- [Concepto Central](#-concepto-central)
- [Los Dos Componentes](#-los-dos-componentes)
- [El Talismán Completo](#-el-talismán-completo)

### **⚔️ Los Componentes**
- [Componente 1: El Cronómetro de Realidades](#componente-1-el-cronómetro-de-realidades)
- [Componente 2: La Perla del Vacío Primordial](#componente-2-la-perla-del-vacío-primordial)

### **🏝️ La Nueva Facción**
- [Los Oceánicos de las Profundidades - Raza de las Profundidades](#-los-océnicos-de-las-profundidades---raza-de-las-profundidades)
- [Historia de la Traición de Teach](#-historia-de-la-traición-de-teach)

### **🎭 Integración Narrativa**
- [Cronología del Robo](#-cronología-del-robo)
- [Pistas para los PJ](#-pistas-para-los-pj)
- [Cómo Prevenir la Creación](#-cómo-prevenir-la-creación)

### **📊 Mecánicas**
- [Ritual de Combinación](#-ritual-de-combinación)
- [Estadísticas del Talismán](#-estadísticas-del-talismán)

---

## 🎯 **CONCEPTO CENTRAL**

### **La Estrategia de Edward Teach:**

Edward Teach es un estratega paciente que ha estado **investigando en secreto** desde que sospechó del plan de Manshoon. Durante su investigación, descubrió que existen dos artefactos que, si se combinan, podrían interceptar el flujo de poder divino.

**⚠️ CRONOLOGÍA IMPORTANTE:**
Edward Teach **NO tiene el Talismán al inicio de la campaña**. Lo creará DURANTE las sesiones después de robar ambos componentes. Los PJ estarán directamente involucrados en estos eventos.

**Los Dos Artefactos:**
- **El Cronómetro de Realidades** (Temporal) - Pertenece a los Anacronistas en Cronópolis
- **La Perla del Vacío Primordial** (Dimensional) - Pertenece a los Oceánicos de las Profundidades en Abysara

**🔗 Conexión Anacronistas-Abysara:**
Los **Anacronistas** tienen **raíces en Abysara**. El **Cronómetro** es **tecnología original de Abysara** que llegó a la superficie a través de contactos secretos. Teach descubrió esta conexión y robó ambos artefactos para crear el Talismán.

**El Plan de Teach:**
1. **Sesiones 1-3:** Investigar discretamente los artefactos
2. **Sesión 4:** Robar el Cronómetro de Realidades (con ayuda de secuaz o traidor)
3. **Sesión 4-5:** Robar la Perla del Vacío Primordial (traición masiva)
4. **Sesión 5:** Combinar los artefactos en el Talismán
5. **Sesión 5-6:** **ASESINAR A THYRA** y robar su poder de estasis temporal
6. **Sesión 6+:** Desencadenar La Ascensión del Cónclave (guerra entre lugartenientes)

Cuando se combinan, crean el **Talismán de Interceptación Divina**, capaz de desviar el flujo de poder divino del ritual de Manshoon hacia su portador.

---

## ⚔️ **LOS DOS COMPONENTES**

### **Componente 1: El Cronómetro de Realidades**

#### **📜 Historia:**

El Cronómetro de Realidades es un artefacto gnomo antiguo creado por los fundadores de los Anacronistas hace más de 500 años. Fue diseñado originalmente para **medir y estabilizar distorsiones temporales**, pero con el tiempo, los Anacronistas descubrieron que también podía **detectar y capturar fragmentos de poder temporal**.

**Ubicación Actual:** Guardado en la **Cámara del Tiempo** en Cronópolis, el cuartel general de los Anacronistas.

**Estado al Inicio de la Campaña:** **SEGURO** - Los Anacronistas lo protegen celosamente. Los PJ pueden verlo durante su visita a Cronópolis.

**Fecha del Robo:** **Sesión 4** - Durante la campaña, después de que los PJ conozcan su existencia.

---

#### **🎨 Descripción Física:**

*"Un reloj de bolsillo gnomo del tamaño de una mano. Su carcasa es de oro envejecido con engranajes visibles a través de cristal de zafiro. Pero los engranajes NO giran normalmente: algunos giran hacia atrás, otros se detienen y arrancan aleatoriamente, y algunos parecen girar en múltiples direcciones simultáneamente. En el centro, donde debería haber manecillas, hay un pequeño cristal que pulsa con luz dorada."*

**Dimensiones:** 4 pulgadas de diámetro, 1 pulgada de grosor
**Peso:** 2 libras (inusualmente pesado para su tamaño)
**Material:** Oro enano antiguo, cristal de zafiro, engranajes de mithril

---

#### **⚡ Poderes del Cronómetro (Solo):**

**Cuando se porta sin combinar:**

**Detección Temporal (Pasivo):**
- El portador puede sentir distorsiones temporales en 1 milla
- Ventaja en salvaciones contra efectos temporales
- Puede ver "ecos" de eventos que sucedieron hace hasta 1 hora

**Captura Temporal (Activo, 3 cargas/día):**
- El portador puede "capturar" un fragmento de tiempo de 6 segundos
- Durante ese tiempo, puede actuar libremente mientras todo lo demás está congelado
- Equivale a *Time Stop* (hechizo de nivel 9)

**Estabilización Temporal (Activo, 1/día):**
- El portador puede revertir completamente 1 turno de combate
- Todo vuelve a como estaba al inicio de ese turno (excepto la memoria del portador)

---

#### **🔐 Cómo Edward Teach Planea Obtenerlo:**

**⚠️ EVENTOS DURANTE LA CAMPAÑA (Sesiones 2-4):**

**Sesión 2-3: Los PJ Conocen a los Anacronistas**

Los PJ llegan a Cronópolis y son recibidos por los Anacronistas. Los líderes, impresionados por las acciones de los PJ contra los Cronófagos, les muestran su cuartel general, incluyendo la **Cámara del Tiempo**.

**Escena Clave - "El Cronómetro Revelado":**

**Líder Anacronista (orgulloso):** *"Este es nuestro tesoro más preciado: el Cronómetro de Realidades. Creado por nuestros fundadores hace 500 años. Puede detectar, medir y estabilizar distorsiones temporales. Con él, hemos mantenido Cronópolis como zona segura durante décadas."*

Los PJ ven el Cronómetro flotando en un pedestal de cristal, protegido por un **Golem Temporal** (CR 12) que nunca duerme.

**Líder:** *"Solo los más confiables conocen su ubicación. Confío en que guardaréis este secreto."*

**⚠️ IMPORTANTE PARA EL DM:**
- Si los PJ **revelan la ubicación** a cualquiera (incluyendo NPCs), esto facilita el robo posterior
- Si los PJ **guardan el secreto**, el robo es más difícil pero aún posible

---

**Sesión 4: El Robo del Cronómetro**

**⚠️ TRES OPCIONES PARA EL DM (Elige la que mejor se adapte a tu mesa):**

---

**OPCIÓN A: Edward Teach Usa un Secuaz**

Teach envía a un **pirata infiltrado** (disfrazado como mercader) que ha estado observando a los PJ. El pirata sigue a los PJ en secreto y descubre la ubicación de la Cámara del Tiempo.

**El Robo:**
1. El pirata espera hasta la noche
2. Usa un dispositivo de portal dimensional (proporcionado por Teach)
3. Teletransporta al Golem Temporal a una dimensión de bolsillo
4. Roba el Cronómetro y escapa

**Testigo:** **Tiber** (Anacronista joven) ve al pirata escapar pero es herido gravemente antes de poder detenerlo.

**Consecuencia:** Los Anacronistas descubren el robo al amanecer. **Los PJ son SOSPECHOSOS** porque conocían la ubicación.

---

**OPCIÓN B: Un Traidor Anacronista**

Un Anacronista de rango medio llamado **Malkor** está secretamente **insatisfecho** con la facción. Cree que el Cronómetro debería usarse activamente contra los Cronófagos, no simplemente guardarse.

Edward Teach (que tiene espías en todas partes) contacta a Malkor y le ofrece un trato: "Roba el Cronómetro, y yo lo usaré para destruir a Aethernus. Después, te lo devolveré."

Malkor, creyendo que esto salvará Chronosia, acepta.

**El Robo:**
1. Malkor, como Anacronista de confianza, tiene acceso a la Cámara
2. Usa un artefacto gnomo para adormecer temporalmente al Golem
3. Toma el Cronómetro y huye de Cronópolis
4. Se encuentra con agentes de Edward Teach en las afueras

**Consecuencia:** Los Anacronistas descubren el robo y la traición. Malkor es perseguido. **Los PJ pueden ser enviados a cazarlo**.

---

**OPCIÓN C: Los PJ son Incriminados (Más Dramático)**

Edward Teach ejecuta el robo personalmente PERO deja evidencia que **inculpa a los PJ**:
- Usa un portal dimensional similar a hechizos que los PJ conocen
- Deja un objeto personal de uno de los PJ en la escena
- El testimonio de Tiber (confundido por magia de encantamiento) describe a alguien parecido a un PJ

**El Robo:**
1. Teach se infiltra en Cronópolis la misma noche que los PJ descansan allí
2. Usa portales dimensionales para entrar a la Cámara
3. Teletransporta al Golem a otra dimensión
4. Roba el Cronómetro y planta evidencia

**Consecuencia:** Los Anacronistas **ARRESTAN A LOS PJ** al amanecer. Los acusan de traición. Los PJ deben probar su inocencia investigando al verdadero ladrón.

---

**Desarrollo Posterior (Cualquier Opción):**

Los PJ pueden:
1. **Investigar el robo** - Pistas les llevan al Archipiélago de Barbanegra
2. **Ser juzgados por los Anacronistas** - Deben demostrar inocencia
3. **Perseguir al ladrón** - Rastro lleva a Edward Teach o su secuaz
4. **Descubrir que Teach está detrás** - Revela su plan maestro

**Pistas Clave:**
- El dispositivo de portal usado tiene firma dimensional única (Teach)
- Testigos vieron un barco pirata cerca de Cronópolis esa noche
- Si capturan al pirata/Malkor, puede confesar bajo interrogatorio

---

### **Componente 2: La Perla del Vacío Primordial**

#### **📜 Historia:**

La Perla del Vacío Primordial es un artefacto de origen desconocido (posiblemente de otro plano) que fue encontrado por los **Oceánicos de las Profundidades** hace milenios en las profundidades oceánicas cerca del Archipiélago de Barbanegra.

Los Oceánicos de las Profundidades, una raza acuática inteligente, adoraban la Perla como su tesoro más sagrado. Creían que contenía un fragmento del **Vacío Primordial** - el espacio antes de que existiera el universo.

**Ubicación Actual:** La **Ciudad Sumergida de Abysara**, capital submarina de los Oceánicos de las Profundidades, a 2 millas de profundidad bajo el Archipiélago de Barbanegra.

**Estado al Inicio de la Campaña:** **SEGURO** - Los Oceánicos de las Profundidades la guardan como su reliquia más sagrada. Desconocen que Edward Teach planea robarla.

**Fecha del Robo:** **Sesión 4-5** - Poco después del robo del Cronómetro.

---

#### **🎨 Descripción Física:**

*"Una perla del tamaño de un puño humano, perfectamente esférica. Su superficie es negra como la obsidiana, pero con iridiscencia púrpura que cambia constantemente. Cuando la observas, sientes que está mirando DENTRO de ti. Si miras fijamente durante demasiado tiempo, ves el vacío absoluto reflejado en tu alma. En su interior, diminutas estrellas parecen brillar y morir en ciclos eternos."*

**Dimensiones:** 4 pulgadas de diámetro
**Peso:** Ingrávida (literalmente no tiene peso - flota si se suelta)
**Material:** Desconocido (no es perla natural, posiblemente sustancia extraplanar)

---

#### **⚡ Poderes de la Perla (Sola):**

**Cuando se porta sin combinar:**

**Vacío Interior (Pasivo):**
- El portador puede respirar en cualquier entorno (agua, vacío, gas venenoso)
- Inmunidad a efectos de presión (profundidades oceánicas, vacío espacial)
- Ventaja en salvaciones contra efectos dimensionales

**Compresión Espacial (Activo, 3 cargas/día):**
- El portador puede comprimir espacio en 60 pies
- Las distancias se reducen a 1/10 (60 pies se convierten en 6 pies)
- Permite moverse enormes distancias en segundos

**Portal al Vacío (Activo, 1/día):**
- El portador abre un portal al Vacío Primordial
- Todos en 30 pies deben hacer TS de Carisma CD 20 o ser succionados
- Los que fallan son desterrados a un plano de vacío absoluto por 1 minuto

---

#### **🔐 Cómo Edward Teach Planea Obtenerla:**

**⚠️ EVENTOS DURANTE LA CAMPAÑA (Sesión 4-5):**

**Contexto:**
Edward Teach tiene una relación comercial con los Oceánicos de las Profundidades desde hace **20 años**. Les ha vendido mercancías de superficie (armas, herramientas, alimentos exóticos) a cambio de tesoros submarinos. Los Oceánicos lo consideran un **aliado confiable** (error fatal).

---

**Sesión 4-5: La Traición de Barbanegra**

**⚠️ TIMING:** Esto ocurre INMEDIATAMENTE después de que Teach obtiene el Cronómetro. Ahora necesita el segundo componente.

**La Petición:**

Teach envía un mensaje a la Reina Nereia: *"Majestad, durante años he comerciado con vuestro pueblo honorable. Ahora, los Cronófagos amenazan mis islas. Solicito permiso para visitar Abysara y pedir consejo sobre cómo defenderme de las distorsiones dimensionales. Vuestro pueblo conoce el espacio mejor que nadie."*

La Reina Nereia, genuinamente preocupada por su aliado de dos décadas, **acepta** la visita.

---

**El Encuentro:**

**Escena - "La Recepción en Abysara"**

Los PJ pueden enterarse de esta visita de varias formas:
- Espías en el Archipiélago mencionan que Teach desapareció bajo el agua
- Los Oceánicos de las Profundidades contactan a los PJ (si ya los conocieron) mencionando la visita
- Los PJ siguen a Teach después del robo del Cronómetro

**La Ceremonia:**

Teach es recibido con honores en el Palacio Real de Abysara. La Reina Nereia le muestra hospitalidad tradicional de los Oceánicos. Durante la ceremonia de bienvenida, Teach ve la **Perla del Vacío Primordial** por primera vez.

**Reina Nereia (orgullosa):** *"Esta es nuestra reliquia más sagrada, Capitán Teach. La Perla del Vacío Primordial. Encontrada por nuestros ancestros en las profundidades hace milenios. Contiene un fragmento del espacio primordial mismo."*

Teach sonríe, pero sus ojos brillan con codicia.

---

**El Robo:**

Esa noche, mientras los Oceánicos de las Profundidades duermen (no todos al mismo tiempo, pero tienen guardias reducidos), Teach actúa.

**⚠️ IMPORTANTE:** Teach NO tiene los poderes de Thyra todavía. Usa sus poderes dimensionales naturales + el Cronómetro recién robado.

**Método:**
1. Usa el **Cronómetro de Realidades** para congelar el tiempo localmente (6 segundos)
2. Durante esos 6 segundos, usa portal dimensional para entrar al Templo del Vacío
3. Toma la Perla del pedestal
4. Sale mediante portal antes de que el tiempo se reanude

**La Alarma:**

Los guardias detectan la perturbación temporal inmediatamente. La Reina Nereia despierta y ordena cerrar todas las salidas de la ciudad.

---

**La Masacre:**

Los Oceánicos de las Profundidades intentan detener a Teach antes de que escape de Abysara. **47 guerreros Oceánicos** liderados por el Comandante Kael lo interceptan en el Gran Salón.

**Comandante Kael (furioso):** *"¡TRAIDOR! ¡Devuelve la Perla AHORA!"*

**Edward Teach (sonriendo oscuramente):** *"Lo siento, amigos. Negocios son negocios. Necesito esto más de lo que vosotros podéis imaginar."*

Teach usa la **Perla del Vacío Primordial** recién robada para abrir un portal al **Vacío Primordial**. Los 47 guerreros son succionados y desaparecen en la nada absoluta.

El Comandante Kael sobrevive (queda fuera del alcance), pero está herido y devastado.

Teach escapa de Abysara usando un portal dimensional a la superficie.

---

**Consecuencia Inmediata:**

**Los Oceánicos de las Profundidades están en LUTO MASIVO y FURIA ABSOLUTA.**

- 47 guerreros muertos (incluyendo algunos de las familias más antiguas)
- Su reliquia más sagrada robada
- Su aliado de 20 años reveló ser un traidor
- La Reina Nereia declara **VENDETTA ETERNA** contra Edward Teach

**Reina Nereia (lágrimas en sus ojos, voz tronadora):** *"Edward Teach, llamado Barbanegra. Has traicionado nuestra confianza. Has asesinado a nuestros hijos. Has robado nuestra alma. Desde este día, eres enemigo de los Oceánicos de las Profundidades por toda la eternidad. Te perseguiremos en cada plano, en cada dimensión. No habrá refugio para ti. Esta es mi promesa."*

---

**Opciones para los PJ:**

**Si los PJ están siguiendo a Teach:**
- Pueden llegar a Abysara justo DESPUÉS de la masacre
- Encuentran al Comandante Kael herido y furioso
- Kael puede aliarse con ellos inmediatamente

**Si los PJ NO siguieron a Teach:**
- Los Oceánicos de las Profundidades contactan a los PJ (si los conocen)
- O los PJ descubren la masacre mediante rumores en el Archipiélago
- Los marineros hablan de "cadáveres que nunca aparecieron" y "el mar llorando"

**Si los PJ llegan DURANTE el robo:**
- Pueden intentar detener a Teach (combate épico)
- Si fallan, presencian la masacre
- Si tienen éxito, previenen el robo (altera drásticamente la campaña)

---

## 🏝️ **LOS OCEÁNICOS DE LAS PROFUNDIDADES - RAZA DE LAS PROFUNDIDADES**

### **📜 Descripción de la Raza:**

Los Oceánicos de las Profundidades son una raza humanoide acuática que vive en las profundidades oceánicas del Archipiélago de Barbanegra. Son seres nobles, pacíficos y extremadamente longevos.

**🌊 Estado Actual - Sociedad Aislada:**
Los Oceánicos de las Profundidades han vivido **reprimidos y aislados** por conflictos antiguos que ya nadie recuerda. Tienen **cierto odio hacia los de la superficie** debido a este aislamiento forzado. Historiadores tienen **evidencias** de que Abysara existe, pero la mayoría la considera una **leyenda**.

**🔗 Conexión con los Anacronistas:**
Los **Anacronistas** tienen **raíces en Abysara**. Hace siglos, algunos Oceánicos escaparon a la superficie y fundaron la facción, llevando consigo **tecnología avanzada** que se convirtió en el **Cronómetro de Realidades**. Esta conexión secreta explica por qué Teach robó ambos artefactos.

---

#### **🎨 Apariencia Física:**

**Características Generales:**
- **Altura:** 6-7 pies (más altos que humanos promedio)
- **Complexión:** Delgados pero musculosos, adaptados para nadar
- **Piel:** Escamas iridiscentes en tonos azul, verde y púrpura
- **Cabeza:** Rasgos humanoides con ojos grandes y luminiscentes (adaptados a la oscuridad)
- **Extremidades:** Manos y pies palmeados con dedos largos
- **Características Únicas:** 
  - Branquias en el cuello que brillan tenuemente
  - Cabello similar a tentáculos o algas bioluminiscentes
  - Marcas faciales que cambian de color según emociones

**Dimorfismo Sexual:**
- **Machos:** Tonos más oscuros (azul profundo, púrpura), crestas óseas en brazos
- **Hembras:** Tonos más claros (turquesa, verde mar), bioluminiscencia más intensa

---

#### **🏛️ Sociedad y Cultura:**

**Estructura Social:**
- **Monarquía Matriarcal:** Gobernados por la **Reina Nereia** (actualmente con 347 años)
- **Consejo de Ancianos:** 12 Oceánicos de más de 200 años asesoran a la Reina
- **Castas:** Guerreros, Artesanos, Eruditos, Recolectores

**Valores Culturales:**
- **Honor y Palabra:** Los Oceánicos de las Profundidades nunca rompen promesas (por eso la traición de Teach fue devastadora)
- **Protección de Tesoros:** Consideran sagrado proteger artefactos antiguos
- **Venganza Justa:** Si son traicionados, perseguirán venganza durante generaciones
- **Pacifismo Condicional:** Evitan violencia, pero son guerreros temibles si provocados
- **Aislamiento Defensivo:** Desconfían de los de la superficie debido a conflictos antiguos
- **Preservación Cultural:** Mantienen su sociedad aislada para proteger su identidad

**Religión:**
- Adoran a **Voidar** (Dios del Espacio) como deidad principal
- Creen que el océano es una manifestación del "espacio líquido"
- La Perla del Vacío Primordial era su reliquia más sagrada

---

#### **🌊 Abysara - La Ciudad Sumergida**

**Descripción:**

*"Abysara es una metrópolis submarina de cristal y coral vivo. Las estructuras crecen orgánicamente del lecho marino, formando torres espirales de hasta 500 pies de altura. Todo brilla con bioluminiscencia: azul, verde, púrpura. Peces y criaturas marinas nadan libremente entre edificios. En el centro, el Palacio Real: una estructura de cristal negro con forma de caracola gigante."*

**Ubicación:** 2 millas bajo la superficie, 10 millas al sur del Archipiélago de Barbanegra

**Población:** ~3,000 Oceánicos de las Profundidades

**Puntos de Interés:**
- **Palacio Real:** Residencia de la Reina Nereia
- **Templo del Vacío:** Donde se guardaba la Perla (ahora vacío y lugar de luto)
- **Academia de Eruditos:** Biblioteca submarina con textos antiguos
- **Barracas de Guerreros:** Donde entrenan los defensores de Abysara

**Acceso:** Imposible llegar sin magia o ayuda de los Oceánicos (presión de 2 millas es mortal)

---

#### **👑 NPCs Importantes:**

**Reina Nereia (Líder de los Oceánicos de las Profundidades)**
- **Edad:** 347 años (mediana edad para los Oceánicos)
- **Apariencia:** 7 pies de altura, piel turquesa con marcas doradas, cabello de tentáculos bioluminiscentes que fluyen como agua
- **Personalidad:** Sabia, paciente, pero FURIOSA por la traición de Teach
- **Motivación:** Recuperar la Perla y vengarse de Teach
- **Información Clave:** Sabe que Teach buscaba la Perla para "un ritual de poder divino"

**Comandante Kael (Líder Militar)**
- **Edad:** 198 años
- **Apariencia:** 6.5 pies, piel azul oscuro con cicatrices, crestas óseas prominentes
- **Personalidad:** Guerrero honorable, protector feroz
- **Motivación:** Lideró el intento de detener a Teach (falló, se culpa a sí mismo)
- **Habilidad:** Podría enseñar a los PJ técnicas de combate submarino

**Archivista Mira (Erudita)**
- **Edad:** 412 años (anciana)
- **Apariencia:** 6 pies, piel verde pálido, cabello casi transparente
- **Personalidad:** Intelectual, curiosa, posee conocimiento antiguo
- **Información Clave:** Conoce la historia completa de la Perla y sus poderes

---

### **🗡️ HISTORIA DE LA TRAICIÓN DE TEACH**

#### **Cronología Completa:**

**Hace 20 Años (Antes de la Campaña):**
- Edward Teach establece contacto comercial con los Oceánicos de las Profundidades
- Intercambia mercancías por tesoros submarinos
- Relación cordial que dura DOS DÉCADAS
- Los Oceánicos lo consideran aliado confiable

**Hace 2 Años (Antes de la Campaña):**
- Teach sospecha que Manshoon (Aethernus) está planeando algo grande
- Comienza a investigar discretamente
- Descubre rumores sobre "ascensión divina"

**Hace 1 Año (Antes de la Campaña):**
- Teach confirma que Manshoon planea convertirse en dios
- Investiga formas de interceptar el poder divino
- Encuentra leyendas antiguas sobre artefactos que capturan esencia divina
- Identifica el Cronómetro de Realidades y la Perla del Vacío Primordial

**Hace 6 Meses (Antes de la Campaña):**
- Teach planea los robos meticulosamente
- Establece espías en Cronópolis
- Comienza a preparar excusas para visitar Abysara

---

**⚠️ INICIO DE LA CAMPAÑA - Los PJ llegan a Chronosia**

**Sesión 1-3:**
- Los PJ investigan el asesinato en Waterdeep
- Llegan a Chronosia
- Conocen a los Anacronistas en Cronópolis
- Ven el Cronómetro de Realidades
- **Thyra la Suspendida está VIVA** (los PJ pueden encontrarse con ella)

**Sesión 4: EL ROBO DEL CRONÓMETRO**
- Edward Teach (o su secuaz/traidor) roba el Cronómetro
- Los Anacronistas descubren el robo al amanecer
- **Los PJ pueden ser SOSPECHOSOS o INVESTIGADORES**
- Rastro lleva al Archipiélago de Barbanegra

**Sesión 4-5: EL ROBO DE LA PERLA**
- Teach visita Abysara (usando el Cronómetro para el robo)
- Roba la Perla del Vacío Primordial
- **MASACRE DE 47 GUERREROS OCEÁNICOS**
- Los Oceánicos de las Profundidades declaran vendetta eterna
- Los PJ pueden descubrir la traición

**Sesión 5: LA COMBINACIÓN DEL TALISMÁN**
- Teach regresa a su isla secreta
- Ejecuta el ritual de combinación (1 hora)
- Crea el **Talismán de Interceptación Divina**
- Los PJ pueden intentar interrumpir (si actuaron rápido)

**Sesión 5-6: EL ASESINATO DE THYRA**
- **⚠️ EVENTO CATALIZADOR DE LA CAMPAÑA**
- Edward Teach, ahora con el Talismán completo, se siente invencible
- Confronta a **Thyra la Suspendida** en su región
- Usa el Talismán para debilitarla temporalmente
- **ASESINA A THYRA** y absorbe sus poderes de estasis temporal
- Los otros lugartenientes se enteran del asesinato
- **COMIENZA LA ASCENSIÓN DEL CÓNCLAVE** (guerra entre lugartenientes)

**Sesión 6: LA REACCIÓN ESPONTÁNEA**
- Los lugartenientes se enteran del asesinato de Thyra (rumores, espías, perturbaciones mágicas)
- **Manshoon NO sabe nada** - está completamente recluido en la Torre de la Eternidad
- Cada lugarteniente reacciona según su personalidad (ver `05_La_Ascension_del_Conclave.md` en `../../02_Guia_DM/05_La_Ascension_del_Conclave.md`)
- Comienzan a formar alianzas y traiciones de forma ORGÁNICA (no por orden de Manshoon)

**Sesión 6-12: FASE SANDBOX**
- Guerra abierta entre lugartenientes (ellos CREEN que están "demostrando su valía" para Aethernus)
- Los PJ enfrentan a los lugartenientes uno por uno
- Edward Teach es ahora el lugarteniente MÁS PODEROSO (tiene Talismán + poderes de Thyra)
- Los Anacronistas y Oceánicos de las Profundidades son aliados potenciales de los PJ

**Sesión 13+: CLÍMAX**
- Teach planea usar el Talismán para robar el poder divino de Manshoon
- Los PJ deben detener tanto a Manshoon como a Teach

---

## 🎭 **INTEGRACIÓN NARRATIVA**

### **📅 CRONOLOGÍA DE LOS EVENTOS (Para el DM)**

**Tabla de Eventos Clave:**

| **Sesión** | **Evento** | **Descripción** |
|-----------|-----------|----------------|
| **2-3** | Los PJ conocen a los Anacronistas | Ven el Cronómetro de Realidades en la Cámara del Tiempo |
| **4** | **ROBO DEL CRONÓMETRO** | Teach/secuaz roba el artefacto, los PJ pueden ser sospechosos |
| **4-5** | **ROBO DE LA PERLA** | Teach traiciona a los Oceánicos, masacre de 47 guerreros |
| **5** | **COMBINACIÓN DEL TALISMÁN** | Teach crea el Talismán (los PJ pueden interrumpir) |
| **5-6** | **⚡ ASESINATO DE THYRA** | Teach mata a Thyra y roba sus poderes (EVENTO CATALIZADOR) |
| **6** | **REACCIÓN ESPONTÁNEA** | Los lugartenientes reaccionan al asesinato (guerra entre lugartenientes) |
| **6-12** | Fase Sandbox | Los PJ enfrentan a los lugartenientes, Teach es el más poderoso |
| **13+** | Clímax | Teach intenta robar el poder divino de Manshoon |

---

### **⚡ EL ASESINATO DE THYRA - EVENTO CATALIZADOR**

**⚠️ SESIÓN 5-6: El Momento que lo Cambia Todo**

#### **Contexto:**

Edward Teach ahora posee:
- El Cronómetro de Realidades (robado en Sesión 4)
- La Perla del Vacío Primordial (robado en Sesión 4-5)
- El Talismán de Interceptación Divina (creado en Sesión 5)

Se siente **invencible** y decide probar su poder contra otro lugarteniente.

---

#### **La Región de Thyra:**

**Thyra la Suspendida** gobierna la **Fortaleza del Momento Congelado**, una región donde el tiempo fluye extremadamente lento. Los habitantes viven en estasis temporal perpetua.

**Características de Thyra:**
- **Poder:** Estasis temporal - puede congelar tiempo localmente
- **Personalidad:** Fría, calculadora, despiadada
- **Relación con Teach:** Neutral (no aliados ni enemigos)
- **CR:** 16 (lugarteniente poderoso pero no el más fuerte)

---

#### **La Confrontación:**

**Escena - "El Momento Congelado"**

Edward Teach navega su flota hacia la región de Thyra. Solicita reunión "diplomática" para "discutir alianza contra los aventureros molestos" (los PJ).

Thyra, intrigada, acepta reunirse en su fortaleza.

**Diálogo:**

**Thyra (voz fría como el hielo):** *"Edward Teach. El pirata dimensional. ¿Qué quieres?"*

**Teach (sonriendo):** *"Quiero ver si este juguete funciona."*

Teach activa el **Talismán de Interceptación Divina**. La energía del Talismán interfiere con los poderes temporales de Thyra, debilitándola momentáneamente.

**Thyra (sorprendida):** *"¿Qué... qué ES eso?"*

**Teach:** *"El futuro. Y tú... eres el pasado."*

---

#### **La Batalla:**

Teach ataca a Thyra con toda su fuerza:
- Usa portales dimensionales para evadir sus ataques
- El Cronómetro contrarresta sus poderes temporales
- La Perla la desorienta con distorsiones espaciales
- El Talismán absorbe gradualmente su energía

Thyra lucha ferozmente pero está en desventaja. Teach es ahora demasiado poderoso.

**Thyra (herida mortalmente):** *"Aethernus... se enterará de esto... te matará..."*

**Teach (riendo):** *"Aethernus está demasiado ocupado jugando a ser dios. No tiene tiempo para nosotros, pequeña Thyra."*

Teach la mata con un golpe final. Su cuerpo se desintegra en cristales de tiempo.

---

#### **La Absorción:**

Cuando Thyra muere, su poder de **estasis temporal** se libera. Normalmente, este poder regresaría a Manshoon. PERO el **Talismán de Interceptación Divina** lo intercepta.

El poder fluye hacia Edward Teach. Su cuerpo brilla con luz dorada y púrpura. Absorbe completamente los poderes de estasis temporal de Thyra.

**Teach (sintiendo el poder):** *"Increíble... FUNCIONA. El Talismán funciona. Ahora... ¿quién sigue?"*

---

#### **Las Consecuencias:**

**1. Los Otros Lugartenientes se Enteran:**
- Noticias del asesinato se esparcen rápidamente
- Los lugartenientes están ATERRORIZADOS
- Algunos buscan aliarse con Teach, otros planean destruirlo
- Varrak (el Oráculo) ve múltiples futuros oscuros

**2. La Reacción de Manshoon:**
- Manshoon (en su torre, ejecutando el ritual) **NO SE ENTERA** del asesinato
- Está completamente recluido, en fase final del ritual de ascensión divina
- **NO proclama nada** - ni siquiera sabe que sus lugartenientes están luchando entre sí
- Su sistema de distribución de poder está en "automático" - fluye al más fuerte sin su intervención

**3. Los PJ Descubren:**
Los PJ pueden enterarse del asesinato mediante:
- **Visión psíquica** (si tienen hechiceros/clarividentes)
- **Rumores en Cronópolis** (Anacronistas detectan perturbación temporal masiva)
- **Mensaje de otro lugarteniente** (si tienen aliados)
- **Encuentro directo** (si estaban cerca de la región de Thyra)

---

#### **Opciones para los PJ:**

**Si llegan DURANTE el asesinato:**
- Pueden intentar prevenir el asesinato (combate épico contra Teach)
- ⚠️ **IMPORTANTE:** Aunque intenten salvarla, **Thyra muere de todas formas**
  - Pueden retrasar el asesinato
  - Pueden herir gravemente a Teach
  - Pero Teach completa el asesinato inevitablemente
- **Consecuencias de intentar prevenir:** Pueden debilitar a Teach, ganar información, o retrasar el proceso, pero Thyra muere

**Si llegan DESPUÉS del asesinato:**
- Encuentran la Fortaleza del Momento Congelado vacía
- Cristales de tiempo esparcidos por el suelo
- Pueden investigar y descubrir que Teach fue el asesino
- Los habitantes congelados de Thyra comienzan a despertar (sin su poder, el tiempo se reanuda)

**Si llegan MUCHO DESPUÉS:**
- La región de Thyra está completamente abandonada
- Los Cronófagos leales a ella se dispersaron o murieron
- Rumores dicen que Edward Teach ahora puede "congelar tiempo como Thyra"

---

### **🔍 PISTAS PRINCIPALES PARA LOS PJ**

**Las pistas ahora están organizadas por la nueva cronología:**

#### **Pista 1: "El Cronómetro de los Anacronistas" (Sesión 2-3)**

**Ubicación:** Cronópolis - Primera visita

**Cómo Descubrirla:** Los Anacronistas muestran su cuartel general a los PJ

**Escena:**
Ver sección "Cómo Edward Teach Planea Obtenerlo" arriba.

**Efecto Narrativo:**
Los PJ aprenden sobre el Cronómetro. Si revelan su ubicación a cualquiera, facilitan el robo posterior.

---

#### **Pista 2: "El Robo del Cronómetro" (Sesión 4)**

**Ubicación:** Cronópolis - Al amanecer

**Cómo Descubrirla:** Los Anacronistas despiertan a los PJ con gritos

**Escena:**

**Líder Anacronista (furioso, irrumpiendo en la habitación de los PJ):** *"¡El Cronómetro! ¡Ha sido ROBADO! ¡Y vosotros erais los únicos forasteros que conocían su ubicación!"*

Los PJ son confrontados. Deben:
- **Demostrar inocencia** (investigar el robo)
- **Huir** (son arrestados si se quedan)
- **Negociar** (ofrecerse a recuperar el Cronómetro)

**Pistas en la Escena del Crimen:**
- El Golem Temporal desapareció (portal dimensional usado)
- Tiber (testigo) describe a alguien con "barba negra" (si Opción A/C)
- Rastros de energía dimensional (firma única de Teach)
- Testigos vieron un barco pirata cerca de Cronópolis esa noche

---

#### **Pista 3: "La Traición en las Profundidades" (Sesión 4-5)**

**Ubicación:** Archipiélago de Barbanegra - Después del robo de la Perla

**Cómo Descubrirla:** Rumores de marineros, o los Oceánicos de las Profundidades contactan directamente

**Escena:**

Ver sección "Cómo Edward Teach Planea Obtenerla" arriba. Los PJ descubren la masacre de 47 guerreros Oceánicos.

---

#### **Pista 4: "El Asesinato de Thyra" (Sesión 5-6)**

**Ubicación:** Región de Thyra o Cronópolis (perturbación detectada)

**Cómo Descubrirla:** Perturbación temporal masiva detectada por los Anacronistas

**Escena:**

**Líder Anacronista (alarmado):** *"Hemos detectado una perturbación temporal catastrófica en la región de Thyra. Algo terrible ha ocurrido. El flujo temporal... se detuvo por completo durante 10 segundos. Eso solo puede significar... que un lugarteniente ha muerto."*

Los PJ pueden viajar a la región de Thyra e investigar.

---

#### **Pista 5: "La Ascensión del Cónclave Comienza" (Sesión 6)**

**Ubicación:** En cualquier lugar de Chronosia

**Cómo Descubrirla:** Mensaje telepático de Manshoon a TODOS en Chronosia

**Escena:**

Ver documento `05_La_Ascension_del_Conclave.md` (en `../../02_Guia_DM/05_La_Ascension_del_Conclave.md`) para las escenas completas de "La Reacción Espontánea" (8 escenas individuales de lugartenientes reaccionando al asesinato de Thyra).

---

### **🛡️ INTENTAR PREVENIR LA CREACIÓN DEL TALISMÁN**

**⚠️ EVENTO FIJO: El Talismán SIEMPRE se crea**

Los PJ pueden intentar prevenir la creación del Talismán, pero **Teach lo completa de todas formas**. Sin embargo, sus acciones pueden tener consecuencias importantes:

---

#### **Opción 1: Robar/Destruir el Cronómetro de Realidades**

**Dificultad:** EXTREMADAMENTE DIFÍCIL

**Ubicación del Cronómetro:** Edward Teach lo guarda en su barco insignia, **La Venganza**, en una cámara secreta protegida por:
- **Trampas dimensionales** (teletransportan intrusos al vacío)
- **Guardias élite** (oficiales Cronófagos leales a Teach)
- **Cerradura mágica** (CD 25 para desactivar)

**Cómo Infiltrarse:**
- Asaltar el barco directamente (combate difícil)
- Infiltrarse cuando Teach no está presente (sigilo CD 22)
- Convencer a un tripulante de traicionar a Teach (persuasión CD 25)

**Si lo recuperan temporalmente:**
- Los Anacronistas estarán EXTREMADAMENTE agradecidos
- Recompensa: Acceso a artefactos gnomicos
- ⚠️ **PERO:** Teach lo recupera más tarde o usa un método alternativo - El Talismán se crea de todas formas

---

#### **Opción 2: Robar/Destruir la Perla del Vacío Primordial**

**Dificultad:** EXTREMADAMENTE DIFÍCIL

**Ubicación de la Perla:** Teach la guarda en su **Dimensión de Bolsillo Personal**, un espacio extradimensional al que solo él puede acceder.

**Cómo Acceder:**
- Capturar a Teach y obligarlo a abrir el portal (requiere derrotarlo)
- Usar magia de nivel 9 (*Gate*, *Wish*) para forzar entrada
- Convencer a Teach de que muestre la Perla (engaño CD 28 - casi imposible)

**Si la recuperan temporalmente:**
- Los Oceánicos de las Profundidades llorarán de alegría
- Recompensa: **Bendición del Vacío** (poder permanente)
- ⚠️ **PERO:** Teach la recupera más tarde o usa un método alternativo - El Talismán se crea de todas formas

---

#### **Opción 3: Interrumpir el Ritual de Combinación**

**Dificultad:** DIFÍCIL (requiere timing perfecto)

**Cuándo Ocurre:** Edward Teach debe combinar los artefactos justo ANTES de ir a la Torre de la Eternidad (Sesión 13).

**Ubicación del Ritual:** Isla secreta en el Archipiélago de Barbanegra (los Oceánicos de las Profundidades pueden guiar a los PJ)

**El Ritual:**
- Teach necesita 1 hora de concentración ininterrumpida
- Debe colocar ambos artefactos en un círculo ritual específico
- Si es interrumpido, el ritual falla y debe esperar 24 horas

**Si intentan interrumpirlo:**
- Combate contra Edward Teach (CR 17) + 6 piratas élite
- ⚠️ **IMPORTANTE:** Aunque interrumpan, Teach completa el ritual de todas formas
  - Puede retirarse y completarlo más tarde
  - Puede completarlo durante el combate si los PJ no lo detienen completamente
  - El Talismán se crea inevitablemente
- **Consecuencias de interrumpir:** Pueden debilitar a Teach, ganar información, o retrasar el proceso, pero el Talismán se crea

---

## 🔮 **EL TALISMÁN COMPLETO**

### **📜 Ritual de Combinación:**

**Requisitos:**
1. El Cronómetro de Realidades
2. La Perla del Vacío Primordial
3. Un círculo ritual inscrito con runas temporales y dimensionales
4. 1 hora de concentración ininterrumpida
5. Sacrificio de un fragmento de poder divino (Teach usa el poder robado de Thyra)

**Proceso:**

1. **Preparación (10 minutos):** Teach dibuja un círculo de 20 pies con runas en Supernal y Abismal
2. **Colocación (5 minutos):** Coloca el Cronómetro en el norte, la Perla en el sur
3. **Invocación (30 minutos):** Canta en idiomas antiguos, canalizando poder
4. **Fusión (15 minutos):** Los artefactos comienzan a flotar y rotar, acercándose gradualmente
5. **Culminación (1 minuto):** Explosión de luz dorada y púrpura, los artefactos se fusionan

**Resultado:**

Los dos artefactos se combinan en uno solo: **El Talismán de Interceptación Divina**.

---

### **🔱 Estadísticas del Talismán de Interceptación Divina**

**Artefacto Legendario (Requiere Sintonización)**

**Descripción Física:**

*"Un medallón del tamaño de una mano. En el centro, la Perla del Vacío Primordial (negra con iridiscencia púrpura). Alrededor, engranajes del Cronómetro de Realidades giran en múltiples direcciones. El objeto pulsa con poder, alternando entre luz dorada (temporal) y oscuridad púrpura (dimensional). Cuando lo sostienes, sientes que el tiempo y el espacio se doblan a tu voluntad."*

---

**Poderes del Talismán:**

**Interceptación Divina (Uso Único):**
- Cuando un ritual de ascensión divina está en progreso a menos de 1 milla de distancia
- El portador puede activar el Talismán como acción
- El flujo de poder divino se DESVÍA del objetivo original hacia el portador
- El portador comienza a absorber el poder divino en lugar del ritualer
- **Duración:** 10 minutos (tiempo suficiente para completar la ascensión si no es interrumpido)

**Efectos de Usar Interceptación Divina:**
- El portador comienza a transformarse en dios (ver "Blackbeard Ascendant")
- Gana +100 HP, AC +5, Resistencia a todo daño
- Cada turno, gana +10% de ascensión divina
- A 100%, la transformación es COMPLETA E IRREVERSIBLE

**Control Temporal-Dimensional (Pasivo mientras se porta):**
- Ventaja en todas las salvaciones
- Puede lanzar *Time Stop* y *Gate* 1/día cada uno
- Inmunidad a efectos temporales y dimensionales

**Maldición:**
- El portador siente la tentación de usar Interceptación Divina
- Cada día, debe hacer TS de Sabiduría CD 20 o quedar *encantado* por el Talismán
- Si falla 3 veces consecutivas, el Talismán controla al portador (busca rituales divinos para interceptar)

---

### **💎 Cómo Destruir el Talismán:**

El Talismán es casi indestructible, pero hay TRES FORMAS:

1. **Separar los Componentes:**
   - Requiere hechizo *Wish* o poder divino equivalente
   - Los artefactos originales se restauran (Cronómetro y Perla)
   - Deben ser devueltos a sus dueños legítimos

2. **Arrojarlo al Vacío Primordial:**
   - Usar el portal que la Perla puede abrir
   - El Talismán regresa a la nada de donde vino
   - Destrucción permanente

3. **Que un Dios lo Destruya:**
   - Amaunator o Voidar pueden destruirlo con un toque
   - Requiere que sean liberados primero
   - El poder se dispersa harmónicamente

---

## 📊 **CONSECUENCIAS NARRATIVAS**

### **Si los PJ Descubren el Plan:**

| **Momento del Descubrimiento** | **Opciones Disponibles** |
|-------------------------------|-------------------------|
| **Sesión 8-9** | Pueden prevenir que Teach complete el ritual de combinación |
| **Sesión 10-11** | Pueden robar uno de los componentes |
| **Sesión 12** | Pueden interrumpir el ritual de combinación |
| **Sesión 13+** | Demasiado tarde - Teach ya tiene el Talismán |

### **⚠️ EVENTO FIJO: El Talismán SIEMPRE se Crea**

**Independientemente de si los PJ descubren el plan:**

- Edward Teach completa el Talismán (con o sin interferencia)
- **Final B depende del timing:** Si Teach llega primero a la Torre, usa el Talismán (Final B)
- Si los PJ llegan primero, Final A o C
- Si llegan simultáneamente, Final C (Carrera Divina)

---

### **Alianzas Resultantes:**

**Si ayudan a los Anacronistas:**
- Recuperan el Cronómetro
- Recompensa: **Artefacto Gnomo** (el DM elige)
- Alianza permanente con los Anacronistas

**Si ayudan a los Oceánicos de las Profundidades:**
- Recuperan la Perla
- Recompensa: **Bendición del Vacío** (resistencia a daño psíquico y fuerza)
- 10 Guerreros Oceánicos se unen para la batalla final

**Si ayudan a ambos:**
- Los Anacronistas y Oceánicos de las Profundidades forman alianza
- Recompensa combinada
- Ejército masivo para el asalto final

---

## 🎭 **ESCENA ÉPICA: "LA COMBINACIÓN"**

**Si los PJ llegan durante el ritual (Sesión 13):**

**Setup:**

Los PJ (guiados por los Oceánicos de las Profundidades) llegan a una isla pequeña y rocosa en medio del océano. Ven humo negro alzándose. Cuando se acercan, encuentran a Edward Teach en el centro de un círculo ritual brillante.

**Descripción:**

*"Edward Teach está de rodillas en el centro de un círculo de runas que brillan con luz dorada y púrpura. Ante él, flotando en el aire: el Cronómetro de Realidades y la Perla del Vacío Primordial. Están girando lentamente, acercándose uno al otro. Teach canta en un idioma que hiere vuestros oídos. Su cuerpo brilla con poder robado. Distorsiones temporales y dimensionales llenan el aire.*

*Cuando os ve, sonríe sin dejar de cantar. Levanta una mano. Seis de sus piratas más leales salen de las sombras, armas en mano.*

*Teach, sin romper el canto, gesticula claramente: 'Intentad detenerme'."*

**Opciones de los PJ:**

1. **Atacar a Teach Directamente:**
   - Los piratas interceptan (combate)
   - Teach continúa el ritual (50% completo)
   - Los PJ tienen 5 turnos para detenerlo

2. **Destruir el Círculo Ritual:**
   - CD 18 Arcana para identificar los puntos débiles
   - Atacar las runas (AC 15, 50 HP cada una, 4 runas)
   - Si destruyen 2+ runas, el ritual falla

3. **Robar los Artefactos:**
   - Los artefactos están protegidos por campo de fuerza
   - *Dispel Magic* nivel 7+ puede romperlo
   - Si los artefactos son movidos, el ritual falla

4. **Negociar:**
   - TS de Persuasión CD 28 (casi imposible)
   - Teach está obsesionado, no puede ser razonado
   - Si se menciona que saben su plan, se enfurece

**Si Teach Completa el Ritual:**

*"Los dos artefactos se tocan. Una explosión de luz dorada y púrpura los ciega. Cuando la luz se disipa, Edward Teach se levanta lentamente, sosteniendo el Talismán de Interceptación Divina. Sonríe con arrogancia absoluta.*

*'Gracias por el espectáculo, héroes. Pero llegasteis tarde. Ahora, si me disculpáis... tengo un dios que destronar.'*

*Teach abre un portal dimensional y desaparece, rumbo a la Torre de la Eternidad."*

---

*El Talismán de Interceptación Divina es la clave del plan de Edward Teach. ⚠️ El Talismán SIEMPRE se crea (evento fijo). Si los PJ descubren su existencia y actúan rápido, pueden debilitar a Teach o retrasar el proceso, pero el Talismán se completa de todas formas. El Final B depende de si Teach llega primero a la Torre de la Eternidad.* 🔱⚡🌊

