# ⏰ FASE 5: VIAJE AL PASADO
## *Sesiones 15-17 - Construcción de la Máquina y Viaje Temporal*

---

> **📖 NAVEGACIÓN:**
> - [← Volver al Índice Principal](../00_Estructura_Campana.md)
> - [Fase 4: Ritual Completado](../Fase_4_Ritual_Completado/00_Guia_General_Fase_4.md)
> - [Fase 6: Batalla Final](../Fase_6_Batalla_Final/00_Guia_General_Fase_6.md)

---

## 📋 **OBJETIVO DE ESTA FASE**

El mundo ha cambiado. Aethernus es dios. Los dioses están muertos. Pero aún hay esperanza. Los Gnomos de Chronosia creen que pueden usar el artefacto de Teach para crear una máquina que viaje en el tiempo. Los PJ deben obtener más anillos de poder, ayudar a construir la máquina salta-mundos, y viajar al pasado para prevenir el ritual.

**⚠️ ESTA FASE ES SEMI-LINEAL** - Los PJ tienen libertad para elegir qué lugartenientes enfrentar, pero deben obtener mínimo 2 anillos más.

---

## 📚 **ARCHIVOS DE ESTA FASE**

### **Sesión 15: El Mundo Post-Ritual**
- **[01_Sesion_15_El_Mundo_Post_Ritual.md](./01_Sesion_15_El_Mundo_Post_Ritual.md)**
- **Nivel:** 9-10
- **Ubicación:** Chronosia (mundo post-apocalíptico)
- **Evento:** Exploración del nuevo mundo, obtención de anillos

### **Sesión 16: Construcción de la Máquina**
- **[02_Sesion_16_Construccion_Maquina.md](./02_Sesion_16_Construccion_Maquina.md)**
- **Nivel:** 10 (nivel máximo de la campaña)
- **Ubicación:** Cronópolis (taller de los Gnomos)
- **Evento:** Construcción de la máquina salta-mundos

### **Sesión 17: El Viaje al Pasado**
- **[03_Sesion_17_El_Viaje_al_Pasado.md](./03_Sesion_17_El_Viaje_al_Pasado.md)**
- **Nivel:** 10 (nivel máximo de la campaña)
- **Ubicación:** Viaje temporal → Pasado (antes del ritual)
- **Evento:** ⚠️⚠️⚠️ **VIAJE TEMPORAL** + **FUSIÓN CON YO DEL PASADO** + **REBATALLA CONTRA TEACH**

---

## 🎯 **CARACTERÍSTICAS DE ESTA FASE**

### **✅ Eventos Fijos:**
- Sesión 15: Exploración del mundo post-ritual, obtención de anillos
- Sesión 16: Construcción de la máquina salta-mundos
- Sesión 17: Viaje al pasado, fusión temporal, rebatalla contra Teach

### **🎲 Elementos Sandbox:**
- Los PJ eligen qué lugartenientes enfrentar para obtener anillos
- Mínimo 2 anillos necesarios (además del Talismán de Teach)
- Los lugartenientes vivos son más poderosos ahora (recibieron poder divino)

### **⚠️ MECÁNICA CRÍTICA - Viaje Temporal:**
- **Fusión Temporal:** Los PJ se fusionan con su "yo del pasado"
- **Nuevas Armas y Conocimientos:** Tienen ventajas en la rebatalla
- **Combate Más Sencillo:** El combate contra Teach es más fácil ahora
- **Llegada a Tiempo:** Llegan a tiempo al ritual (esta vez)

---

## 🔗 **REFERENCIAS IMPORTANTES**

### **Documentos del DM:**
- **[07_Cronologia_Maestra_Campana.md](../../02_Guia_DM/07_Cronologia_Maestra_Campana.md)** - Timeline maestro
- **[06_El_Talisman_de_Interceptacion.md](../Fase_1_Robos_Traicion/06_El_Talisman_de_Interceptacion.md)** - Sistema del Talismán
- **[04_Cronofagos_Detallado/](../../02_Guia_DM/04_Cronofagos_Detallado/)** - Fichas de lugartenientes (ahora más poderosos)

---

## 📅 **TIMELINE DE ESTA FASE**

### **📖 SESIÓN 15: "El Mundo Post-Ritual"**

**El Nuevo Mundo:**
- Chronosia es un campo de batalla multidimensional
- Portales a otros planos están abiertos por todas partes
- Criaturas de otros planos invaden constantemente
- Los lugartenientes vivos son más poderosos (CR +2-3)

**Objetivos:**
- Explorar el nuevo mundo
- Enfrentar lugartenientes vivos para obtener anillos
- Mínimo 2 anillos necesarios (además del Talismán)
- Sobrevivir en un mundo extremadamente peligroso

**Nivel de los PJ:** 9-10

---

### **📖 SESIÓN 16: "Construcción de la Máquina"**

**Los Gnomos Construyen:**
- Din Goldgear y los Gnomos de Chronosia construyen la máquina salta-mundos
- Usan el artefacto de Teach como componente clave
- Requiere energía divina (anillos de poder)
- Forma de tren que abre portales

**Objetivos:**
- Ayudar en la construcción (opcional)
- Proteger a los Gnomos durante la construcción
- Preparar para el viaje temporal

**Nivel de los PJ:** 10 (nivel máximo)

---

### **📖 SESIÓN 17: "El Viaje al Pasado"**

**⚠️⚠️⚠️ VIAJE TEMPORAL:**

**Parte 1: El Viaje (30 minutos)**
- Los PJ entran en la máquina salta-mundos
- El viaje temporal consume las fuentes de poder (anillos)
- Viajan al pasado, justo antes de la batalla contra Teach

**Parte 2: La Fusión Temporal (30 minutos)**
- Los PJ se encuentran con su "yo del pasado"
- Se fusionan con sus versiones pasadas
- Obtienen nuevas armas y conocimientos
- Reviven el momento de la batalla contra Teach

**Parte 3: La Rebatalla contra Teach (2-3 horas)**
- Combate contra Teach (más sencillo ahora)
- Los PJ tienen ventajas por conocimiento previo
- Derrotan a Teach más fácilmente
- Llegan a tiempo al ritual

**Transición a Fase 6:**
- Los PJ llegan a tiempo al ritual
- El ritual aún no se ha completado
- La batalla final puede comenzar

---

## 💡 **CONSEJOS PARA EL DM**

### **El Mundo Post-Ritual:**
- **Peligroso:** Todo es más difícil ahora
- **Lugartenientes Potenciados:** Son más fuertes, más agresivos
- **Portales:** Criaturas de otros planos constantemente
- **Urgencia:** Pero aún hay esperanza

### **La Construcción:**
- **Colaborativa:** Los PJ pueden ayudar
- **Protección:** Deben proteger a los Gnomos
- **Tiempo:** No demasiado tiempo, pero suficiente

### **El Viaje Temporal:**
- **Épico:** Es un momento único
- **Fusión:** Explica claramente la mecánica de fusión
- **Ventajas:** Los PJ tienen ventajas claras
- **Combate Más Fácil:** Pero aún desafiante

---

## 🔄 **TRANSICIÓN A FASE 6**

Después del viaje temporal:
- Los PJ llegan a tiempo al ritual
- El ritual aún no se ha completado
- Pueden intentar prevenir el ritual
- La batalla final puede comenzar

**Siguiente Fase:** [Fase 6: Batalla Final](../Fase_6_Batalla_Final/00_Guia_General_Fase_6.md)

---

## 📖 Cómo explicar el viaje en juego (guía para el DM)

Esta sección ayuda a dirigir la Fase 5 sin contradicciones y a que los jugadores no sientan que "repiten" el mismo combate sin sentido.

### **Qué sabe cada "yo"**

| Quién | Qué sabe | Qué no sabe |
|-------|----------|-------------|
| **Yo del pasado** (antes de la fusión) | Todo hasta el momento justo antes de la batalla contra Teach: que van hacia la Torre, que el ritual está muy avanzado, que Teach es una amenaza. | Que llegarán tarde, que el ritual se completará, que Aethernus se convertirá en dios, que el mundo cambiará, que luego viajarán en el tiempo. |
| **Yo del futuro** (los PJ que llegan en la máquina) | Todo: ritual completado, mundo post-apocalíptico, construcción de la máquina, viaje temporal. | Nada relevante que el pasado no tenga ya (objetivos, aliados, etc.). |
| **Yo fusionado** (después de la fusión) | **Todo:** memorias del pasado (hasta ese instante) + memorias del futuro (ritual, mundo roto, viaje). Es una sola mente con dos líneas de experiencia. | Nada; la fusión combina ambas líneas. |

**En juego:** Decidir que la fusión **no** crea dos conciencias en un cuerpo. Hay un solo "yo" que de repente **recuerda** también el futuro que vivió. Así evitas debates de "¿quién controla el cuerpo?" y los PJ siguen siendo ellos mismos con más información.

### **Qué recuerdan tras la fusión**

- **Sí recuerdan:** El ritual completado, la muerte de los dioses, el mundo post-ritual, la construcción de la máquina, el viaje en el tiempo, las tácticas de Teach que ya vivieron (por eso el combate es "más fácil").
- **Siguen siendo los mismos personajes:** Mismos rasgos, clase, nivel; no se duplican estadísticas. La ventaja es narrativa y de información (conocen el terreno, las habilidades de Teach, el timing).
- **No recuerdan "otro yo" como persona distinta:** La fusión se puede narrar como "de repente recordáis también todo lo que pasó después". No hace falta explicar paradojas; es magia temporal de la máquina.

### **Cómo evitar contradicciones**

1. **Una sola línea temporal jugable:** Tras el viaje, solo existe "el pasado al que han llegado". El futuro del que vienen deja de ser el futuro que ocurrirá si ganan; no hace falta aclarar si "se borra" o "sigue en otro plano". Los PJ están **aquí y ahora** en el pasado, con sus recuerdos del futuro.
2. **Teach en el pasado:** Es el mismo Teach (mismo Talismán, mismo poder) pero los PJ ya lo han enfrentado una vez y conocen sus trucos; por eso el combate es más manejable. No hace falta cambiar sus estadísticas si no quieres; la ventaja puede ser solo narrativa (ventaja en tiradas, CD reducidas, etc.).
3. **NPCs del pasado:** Los NPCs (Gnomos, Anacronistas, etc.) en el pasado son los de ese momento. No "recuerdan" el futuro; para ellos los PJ acaban de llegar. Los PJ no deben contarlo todo para no enredar la escena; el DM puede decir: "Sabéis que explicar el viaje en el tiempo a todo el mundo sería largo y difícil de creer; os centráis en llegar a tiempo al ritual."

### **Frase tipo para narrar la fusión**

*"Vuestras mentes se unen. No sois dos personas en un cuerpo: sois la misma persona que de repente recuerda también todo lo que va a pasar — el ritual, la derrota, el mundo roto, la máquina, este viaje. Esos recuerdos os dan certeza y ventaja: sabéis cómo lucha Teach y qué debe pasar si falláis. Ahora podéis cambiar eso."*

### **Si preguntan "¿no estamos repitiendo el mismo combate?"**

- **Respuesta in-world:** "No es repetición: es una segunda oportunidad. La primera vez llegasteis tarde. Esta vez estáis aquí antes de que el ritual se complete. El combate es el mismo enemigo, pero vosotros sois más sabios y llegáis a tiempo."
- **En mecánicas:** Puedes dar ventaja en iniciativa, en la primera ronda contra Teach, o en tiradas de Inteligencia/Sabiduría relacionadas con sus tácticas. Así se siente que "ya lo conocen" sin cambiar mucho el statblock.

---

*El mundo ha cambiado. Pero aún hay esperanza. Si pueden viajar al pasado... si pueden llegar a tiempo... tal vez puedan cambiar el destino.* ⏰🚂✨


