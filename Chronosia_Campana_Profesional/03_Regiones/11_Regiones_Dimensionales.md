# 🌌 Regiones Dimensionales
## *Los Dominios de los Lugartenientes del Espacio*

---

## 📋 **ÍNDICE DE NAVEGACIÓN**

### **🌌 Las Regiones Dimensionales**
- [Archipiélago de Barbanegra (Edward Teach)](#-archipiélago-de-barbanegra-edward-teach-barbanegra)
- [Las Llanuras de la Compresión (Dimensionalis la Fracturada)](#-las-llanuras-de-la-compresión-dimensionalis-la-fracturada)
- [Las Calderas Dimensionales (Ignis el Devorador Espacial)](#-las-calderas-dimensionales-ignis-el-devorador-espacial)
- [La Ciudad Subterránea de Veldrisza (Matrona Veldrisza)](#-la-ciudad-subterránea-de-veldrisza-matrona-veldrisza)
- [El Jardín de los Tiempos Gemelos (Las Gemelas del Espejo)](#-el-jardín-de-los-tiempos-gemelos-las-gemelas-del-espejo)
- [Glacialis (Ymir el Eterno Invierno - Reclutador Interplanar)](#-glacialis-ymir-el-eterno-invierno)

---

## 🌌 **LAS REGIONES DIMENSIONALES**

Estas regiones están dominadas por lugartenientes que controlan aspectos específicos del espacio.

**⚠️ ESTADO ACTUAL:** El Archipiélago de Edward Teach está **CERRADO** al inicio porque el mar está en calma muerta; se abre al construir el **Motor de Viento Perpetuo** (= Rotor Infinito = derrotar **Serapis + Ymir**). Los **6 lugartenientes dimensionales** están activos.

**⏳ MOTOR DE PRESIÓN:** El ritmo lo marca el **Reloj del Ritual** (la ascensión de Aethernus), no un balance entre tipos de lugarteniente. Derrotar a un lugarteniente crítico (aquí, **Ignis**, líder dimensional, o **Teach**) lo frena. Ver [Motor de Campaña](../02_Guia_DM/10_Motor_de_Campana_Reloj_y_Puertas.md).

**👑 LUGARTENIENTES DIMENSIONALES (6):**
- Edward Teach "Barbanegra" (Independiente - CERRADO hasta el Motor de Viento; los PJ le llegan por mar hacia nivel 7-9)
- Dimensionalis la Fracturada (Independiente)
- Ignis el Devorador Espacial (Líder del Bando Dimensional)
- Matrona Veldrisza "La Tejedora Dimensional" (Bando Dimensional)
- Las Gemelas del Espejo - Lyra y Nyx (Neutrales hasta Sesión 11)
- Ymir el Eterno Invierno (Dimensional)

**📖 Ver documento completo de la competencia:** 05_La_Ascension_del_Conclave.md

---

## 🏴‍☠️ **Archipiélago de Barbanegra (Edward Teach "Barbanegra")**

**Ubicación**: Islas del Oeste, mar abierto

**ESTADO:** 🔒 **CERRADO** (mar en calma muerta) → 🔓 **ABIERTO** cuando La Resistencia construye el **Motor de Viento Perpetuo** (= Rotor Infinito = derrotar **Serapis + Ymir**). NO depende del asesinato de Thyra

**🎖️ LUGARTENIENTE ACTIVO Y CRÍTICO** - Motor de la trama; villano final alternativo. Crece absorbiendo a los lugartenientes que los PJ no alcanzan primero

**⚔️ INDEPENDIENTE** - No se alinea con ningún bando, ha asesinado a Thyra y robado sus poderes

**⚠️ PUNTO CIEGO DE VARRAK:** Edward Teach es invisible a las visiones proféticas de Varrak del Horizonte debido al poder robado de Thyra.

**⚔️ SOLDADOS ÚNICOS:** Edward Teach entrena y otorga poderes de **Estasis Temporal + Manipulación Espacial** a sus soldados, llamados "Los Estancados". Estos soldados pueden congelar parcialmente a los enemigos, crear portales piratas para teletransportarse, y atacar desde múltiples direcciones. Ver [13_Soldados_y_Ejercitos_Regionales.md](../02_Guia_DM/04_Cronofagos_Detallado/13_Soldados_y_Ejercitos_Regionales.md) para detalles completos.

### **Descripción Geográfica**
Un archipiélago de islas piratas dispersas en el Mar Occidental, donde Edward Teach ha establecido su dominio. Las islas están conectadas por portales dimensionales inestables que permiten viaje instantáneo entre ellas. El mar está plagado de barcos fantasma que patrullan constantemente, y en el centro se encuentra la fortaleza flotante del propio Teach, rodeada por una flota de navíos gigantes.

### **Condiciones Especiales**
- Portales dimensionales conectan las islas constantemente
- Los barcos fantasma interceptan cualquier navegante
- El mar cambia de dirección aleatoriamente
- Las islas flotan y cambian de posición
- **Mecánica Especial:** Interceptación obligatoria por barco gigante, colisión y abordaje

### **Habitantes**
- **Edward Teach "Barbanegra"**: El pirata con poderes robados de Thyra
- **Tripulación Pirata**: Navegantes dimensionales corrompidos
- **Guardianes de Barcos**: Criaturas que protegen la flota
- **Prisioneros Temporales**: Atrapados en estasis después del asesinato de Thyra

### **Puntos de Interés**
- **La Fortaleza Flotante**: Residencia principal de Edward Teach
- **El Barco Gigante**: Que intercepta a todos los visitantes
- **El Puerto de las Almas**: Donde los barcos fantasma atracan
- **La Cueva del Tesoro**: Donde se almacenan los tesoros robados
- **El Salón de Trofeos**: Donde están los despojos de Thyra

### **Mecánicas de Juego**
- **Mar en Calma**: El mar alrededor del Archipiélago está completamente en calma, sin viento. Es imposible navegar sin el Motor de Viento Perpetuo
- **Desbloqueo**: La región se desbloquea cuando La Resistencia (específicamente Din Goldgear y los Ingenieros) inventa el **Motor de Viento Perpetuo** usando el Rotor Infinito completo
- **Requisito para el Motor**: Los PJ deben derrotar a Serapis (Núcleo Temporal) Y Ymir (Carcasa Helada) para obtener el Rotor Infinito completo
- **Construcción**: Los Ingenieros construyen el Motor de Viento Perpetuo en 2 semanas (requiere 10,000 po en materiales adicionales)
- **Batalla Naval Obligatoria**: Una vez en el Archipiélago, los PJ deben enfrentar el barco gigante antes de acceder a la fortaleza de Teach
- **Portales Dimensionales**: Permiten movimiento rápido entre islas
- **Poder Robado**: Edward tiene habilidades de estasis temporal + portales dimensionales
- **Estado de Acceso:** 🔒 **BLOQUEADA** hasta que La Resistencia construya el Motor de Viento Perpetuo

### **Efectos Específicos de Edward Teach**

**Poderes Piratas Originales:**
- **Ataque de Cañón**: Dispara cañones que causan daño de área
- **Portal de Intercambio**: Intercambia posiciones con enemigos
- **Desplazamiento Masivo**: Teletransporta a todos los enemigos
- **Portal de Invocación**: Trae criaturas aleatorias de otros planos

**Poderes Robados de Thyra (Post-Sesión 6):**
- **Estasis Pirata**: Se congela en el tiempo, ganando inmunidad total
- **Suspensión de Ataque**: Congela ataques enemigos en el aire
- **Congelación Temporal**: Convierte enemigos en cristal temporal
- **Campo de Estasis**: Crea áreas donde todos pierden su turno
- **Momento Robado**: Roba el turno de un enemigo
- **Congelación Masiva**: Congela el tiempo en un área masiva

**Combinaciones Devastadoras:**
- **Portal Congelado**: Teletransporta y congela al enemigo simultáneamente
- **Estasis y Reposición**: Se hace inmune e intercambia posición con enemigo
- **Desplazamiento Temporal**: Teletransporta y congela a todos
- **Cañón Suspendido**: Daño masivo a enemigos congelados (doble daño)

### **Acciones Legendarias**
Ver [00_Cronofagos_Detallado.md](../02_Guia_DM/04_Cronofagos_Detallado/00_Cronofagos_Detallado.md) para la lista completa de acciones legendarias escalables por nivel, incluyendo la tabla de criaturas invocables.

---

## 🏔️ **Las Llanuras de la Compresión (Dimensionalis la Fracturada)**

**Ubicación**: Región sur-oriental, llanuras extensas con distorsiones espaciales

**🎖️ LUGARTENIENTE ACTIVO** - Participa en "La Ascensión del Cónclave"

**⚔️ INDEPENDIENTE** - Juega sus propias cartas en la competencia

**⚔️ SOLDADOS ÚNICOS:** Dimensionalis entrena y otorga poderes de **Fracturación Dimensional** a sus soldados, llamados "Los Fracturados". Estos soldados pueden existir en múltiples dimensiones simultáneamente, teletransportarse a través de fracturas, y atacar desde múltiples planos. Ver [13_Soldados_y_Ejercitos_Regionales.md](../02_Guia_DM/04_Cronofagos_Detallado/13_Soldados_y_Ejercitos_Regionales.md) para detalles completos.

### **Descripción Geográfica**
Vastas llanuras donde el espacio se ha comprimido hasta límites imposibles. Las distancias se reducen drásticamente, los objetos se comprimen a la mitad de su tamaño, y las proporciones se distorsionan constantemente. Los viajeros pueden volverse gigantes o enanos según la compresión espacial. En el centro se alza una ciudadela de cristal que parece comprimirse y expandirse con el ritmo de un corazón.

### **Condiciones Especiales**
- Las distancias se reducen a la mitad en toda la región
- Los objetos se comprimen pero mantienen su masa original
- Las criaturas pueden volverse gigantes o diminutas temporalmente
- El espacio se fractura en múltiples dimensiones superpuestas

### **Habitantes**
- **Dimensionalis la Fracturada**: Tiefling con cuerpo fragmentado en múltiples dimensiones
- **Guardianes Dimensionales**: Soldados adaptados a la compresión espacial
- **Viajeros Comprimidos**: Criaturas atrapadas en el espacio distorsionado
- **Ecos Dimensionales**: Manifestaciones de otros planos

### **Puntos de Interés**
- **La Ciudadela de Cristal**: Residencia principal de Dimensionalis
- **El Campo de Compresión**: Donde el espacio se comprime al máximo
- **El Portal de Fracturas**: Que conecta con múltiples dimensiones
- **El Laberinto Espacial**: Donde las distancias cambian constantemente

### **Mecánicas de Juego**
- **Compresión de Objetos**: Los objetos se comprimen a la mitad de su tamaño
- **Distorsión de Distancia**: La distancia entre dos puntos se reduce drásticamente
- **Inversión de Proporciones**: Un PJ se vuelve gigante o diminuto temporalmente
- **Separación Dimensional**: Los PJ pueden ser separados en habitaciones dimensionales
- **Estado de Acceso:** 🔒 **Bloqueada** - Requiere derrotar a Ignis el Devorador Espacial

### **Efectos Específicos de Dimensionalis la Fracturada**
- **Fractura Menor**: Crea una pequeña fractura dimensional que confunde
- **Superposición**: Se superpone con versiones de otras dimensiones
- **Laberinto Dimensional**: Crea un laberinto espacial de 30x30 pies
- **Fractura Masiva**: Fractura la realidad en múltiples dimensiones
- **Dimensión Espejo**: Crea una dimensión espejo que causa confusión
- **Superposición Total**: Se superpone con múltiples versiones dimensionales
- **Caos Dimensional**: Fractura completamente la realidad en un área
- **Fragmentación Absoluta**: Se fragmenta en múltiples versiones simultáneas
- **Separación Dimensional**: Separa a los PJ en habitaciones con criaturas

**Mecánica Especial - Separación Dimensional:**
- **Nivel 5-8:** Separa en parejas (2-3 habitaciones)
- **Nivel 9+:** Separa individualmente (1 habitación por jugador)
- **Criaturas:** Aumentan en poder según el nivel de los PJ

### **Acciones Legendarias**
Ver [00_Cronofagos_Detallado.md](../02_Guia_DM/04_Cronofagos_Detallado/00_Cronofagos_Detallado.md) para la lista completa de acciones legendarias escalables por nivel.

---

## 🔥 **Las Calderas Dimensionales (Ignis el Devorador Espacial)**

**Ubicación**: Región volcánica del sur, cerca de Volcanis

**🎖️ LUGARTENIENTE ACTIVO** - Participa en "La Ascensión del Cónclave"

**⚔️ LÍDER DEL BANDO DIMENSIONAL** - Comanda "Los Conquistadores de Planos"

**🌌 EJÉRCITO INVOCADO:** Ignis **NO entrena soldados**. En su lugar, invoca criaturas directamente del **Plano Elemental del Fuego y los Nueve Infiernos** a través de portales dimensionales. Trae dragones rojos (crías y adultos), elementales de fuego, demonios, y otras criaturas infernales que luchan por él pero mantienen sus habilidades originales. **Ignis abre estos portales sin razón aparente, simplemente para sembrar el caos.** Ver [13_Soldados_y_Ejercitos_Regionales.md](../02_Guia_DM/04_Cronofagos_Detallado/13_Soldados_y_Ejercitos_Regionales.md) para detalles completos.

**💎 CRISTALES DE PODER:**
Cerca de la región de Ignis (Las Calderas Dimensionales) es donde **más cristales de poder hay en todo Chronosia**. Esta es una **fuente importante para La Resistencia**, pero la zona es **extremadamente peligrosa** debido a los portales constantes a los Infiernos y las criaturas infernales que trae Ignis sin razón aparente.

### **Descripción Geográfica**
Una región volcánica devastada donde el espacio se ha fracturado en múltiples dimensiones, creando calderas que conectan con el Plano Elemental del Fuego. Los volcanes escupen lava dimensional que se extiende a través de múltiples planos, y las grietas espaciales permiten que el fuego elemental fluya libremente. La región está dominada por el dragón rojo ancestral Ignis, que ha convertido su guarida en un nexo dimensional.

### **Condiciones Especiales**
- Portales dimensionales se abren constantemente, conectando con el Plano Elemental del Fuego
- La lava fluye a través de múltiples dimensiones simultáneamente
- El calor es tan intenso que distorsiona el espacio y el tiempo
- Las grietas espaciales permiten viajar instantáneamente entre ubicaciones distantes

### **Habitantes**
- **Ignis el Devorador Espacial**: El dragón rojo ancestral que domina la región
- **Elementales de Fuego Dimensionales**: Criaturas que existen en múltiples planos
- **Guardianes de Calderas**: Que protegen los portales dimensionales
- **Navegantes del Fuego**: Que pueden moverse entre dimensiones a través del fuego
- **Crías de Dragón Dimensional**: Dragones jóvenes con poderes espaciales

### **Puntos de Interés**
- **La Caldera Central**: El portal más poderoso al Plano Elemental del Fuego
- **La Guarida de Ignis**: Fortaleza dimensional del dragón rojo
- **El Bosque de Lava**: Donde los árboles arden eternamente en múltiples dimensiones
- **El Laberinto de Fuego**: Donde las llamas crean portales dimensionales

### **Mecánicas de Juego**
- **Portales de Fuego e Infiernos**: Se abren constantemente conectando con el Plano Elemental del Fuego y los Nueve Infiernos
- **Influencia en los Infiernos**: Ignis influye su poder en los Nueve Infiernos, abriendo portales allí para traer criaturas sin razón aparente
- **Navegación Dimensional**: Los PJ pueden viajar entre dimensiones a través del fuego
- **Distorsión Espacial**: El calor intenso distorsiona el espacio
- **Invocación de Crías**: Ignis puede convocar dragones rojos, elementales, y demonios
- **Cristales de Poder**: La región tiene la mayor concentración de cristales de poder en Chronosia (importante para La Resistencia pero extremadamente peligrosa)
- **Estado de Acceso:** 🔒 **Bloqueada** - Accesible desde el inicio pero extremadamente peligrosa

### **Efectos Específicos de Ignis el Devorador Espacial**
- **Portal de Ataque**: Crea un portal detrás de un enemigo y ataca a través de él
- **Portal de Crías**: Crea un portal que lanza 1d2 crías de dragón rojo
- **Portal Múltiple**: Crea 2 portales que permiten atacar simultáneamente
- **Portal de Elementales**: Trae 1d4 elementales de fuego
- **Portal de Crías Masivo**: Lanza 1d4+1 crías de dragón rojo
- **Portal de Refuerzos**: Trae 1d3 dragones rojos adultos
- **Portal Infernal**: Crea múltiples portales que atacan desde todas las direcciones
- **Portal de Devastación**: Lanza un ejército de dragones y elementales

### **Acciones Legendarias**
Ver [00_Cronofagos_Detallado.md](../02_Guia_DM/04_Cronofagos_Detallado/00_Cronofagos_Detallado.md) para la lista completa de acciones legendarias escalables por nivel.

---

## 🕷️ **La Ciudad Subterránea de Veldrisza (Matrona Veldrisza "La Tejedora Dimensional")**

**Ubicación**: Región subterránea, asentamiento drow bajo tierra

**🎖️ LUGARTENIENTE ACTIVO** - Participa en "La Ascensión del Cónclave"

**⚔️ BANDO DIMENSIONAL** - Aliada de Ignis en "Los Conquistadores de Planos"

**🌌 EJÉRCITO INVOCADO:** Veldrisza **NO entrena soldados**. En su lugar, invoca criaturas directamente de **Menzoberranzan** a través de portales dimensionales. Trae guerreros drow, sacerdotisas de Lolth, y demonios drow (yochlol) que luchan por ella pero mantienen sus habilidades originales de Menzoberranzan. Ver [13_Soldados_y_Ejercitos_Regionales.md](../02_Guia_DM/04_Cronofagos_Detallado/13_Soldados_y_Ejercitos_Regionales.md) para detalles completos.

### **Descripción Geográfica**
Una ciudad subterránea drow construida en una red de cavernas naturales que se extiende a través de múltiples dimensiones. La ciudad está tejida con hilos de telaraña mágica que conectan con Menzoberranzan, creando portales dimensionales que permiten viajar entre ambos mundos. Las estructuras están talladas en piedra negra y decoradas con patrones de telaraña que brillan con energía espacial.

### **Condiciones Especiales**
- Portales dimensionales conectan constantemente con Menzoberranzan
- Las telarañas mágicas crean redes espaciales que atrapan intrusos
- La oscuridad es absoluta excepto por la luz de los portales dimensionales
- El aire está cargado con energía espacial y magia drow

### **Habitantes**
- **Matrona Veldrisza**: La líder drow con poderes espaciales
- **Familia de la Matrona**: Hijas sacerdotisas e hijos guerreros
- **Ejército Drow**: Guerreros, sacerdotisas y criaturas serviles
- **Drideres Dimensionales**: Guardianes que pueden viajar entre dimensiones
- **Invocados de Menzoberranzan**: Tropas que llegan a través de los portales

### **Puntos de Interés**
- **El Templo de Lolth**: Donde se realizan rituales espaciales
- **La Red de Portales**: Centro de conexión con Menzoberranzan
- **El Palacio de la Matrona**: Residencia principal con poderes dimensionales
- **El Laberinto de Telarañas**: Donde las redes espaciales atrapan enemigos

### **Mecánicas de Juego**
- **Portales Dimensionales**: Se abren constantemente conectando con Menzoberranzan
- **Redes Espaciales**: Las telarañas pueden atrapar y transportar criaturas
- **Magia Drow**: Combinada con poderes espaciales para efectos únicos
- **Invocación de Ejércitos**: Puede traer tropas drow desde Menzoberranzan
- **🕷️ Control de los Bregan Daerthe**: Veldrisza tiene **comprados a los Bregan Daerthe por encima de Serapis**. Aunque los Bregan Daerthe pueden trabajar para Serapis temporalmente, su verdadera lealtad es con Veldrisza. Esta relación secreta le permite manipular eventos desde las sombras.
- **Estado de Acceso:** 🔒 **Bloqueada** - Requiere derrotar a Edward Teach "Barbanegra"

### **Efectos Específicos de Matrona Veldrisza**
- **Red de Telaraña**: Crea una red de telarañas dimensionales que atrapa enemigos
- **Portal a Menzoberranzan**: Abre un portal que trae 1d3 drow guerreros
- **Dimensión de Pesadilla**: Atrapa a un enemigo en una dimensión de pesadilla
- **Red Masiva**: Crea una red que atrapa a todos los enemigos en 60 pies
- **Portal de Ejército**: Trae un ejército drow (2d4 guerreros + 1 sacerdotisa)
- **Dimensión Colectiva**: Atrapa a todos en una dimensión de pesadilla colectiva
- **Red Cósmica**: Crea una red que cubre 120 pies
- **Portal de Lolth**: Abre un portal directo a Lolth, trayendo su poder divino

### **Acciones Legendarias**
Ver [00_Cronofagos_Detallado.md](../02_Guia_DM/04_Cronofagos_Detallado/00_Cronofagos_Detallado.md) para la lista completa de acciones legendarias escalables por nivel.

---

## 🪞 **El Jardín de los Tiempos Gemelos (Las Gemelas del Espejo - Lyra y Nyx)**

**Ubicación**: Región central, compartida con Medusa la Eterna (Temporal)

**🎖️ LUGARTENIENTES ACTIVAS** - Participan en "La Ascensión del Cónclave"

**⚔️ NEUTRALES** - No se alinean con ningún bando hasta Sesión 11

**⚠️ REGIÓN COMPARTIDA:** Esta región puede tener tanto a Las Gemelas del Espejo (Dimensionales) como a Medusa la Eterna (Temporal). El DM decide si coexisten o solo una controla la región.

**⚔️ SOLDADOS ÚNICOS:** Las Gemelas entrenan y otorgan poderes de **Flujo Temporal Bidireccional** (soldados temporales "Los Gemelos") y **Dimensiones Espejo** (soldados dimensionales "Los Reflejados"). Los soldados temporales pueden acelerar o revertir acciones, mientras que los dimensionales pueden crear reflejos y atacar desde dimensiones espejo. Ver [13_Soldados_y_Ejercitos_Regionales.md](../02_Guia_DM/04_Cronofagos_Detallado/13_Soldados_y_Ejercitos_Regionales.md) para detalles completos.

### **Descripción Geográfica**
Un jardín extenso lleno de espejos de diferentes tamaños y formas que actúan como portales dimensionales. La luz se refleja en todos los espejos, creando una atmósfera de irrealidad y duplicación. En el centro se encuentra el santuario de las gemelas, donde la dimensión espejo existe al otro lado de cualquier superficie reflectante. Algunos espejos muestran reflejos vacíos, mientras que otros revelan acciones diferentes de los visitantes.

### **Condiciones Especiales**
- Los espejos actúan como portales a la dimensión espejo
- Los reflejos pueden cobrar vida y actuar independientemente
- La realidad y su reflejo se superponen constantemente
- Todo en el jardín es simétricamente opuesto

### **Habitantes**
- **Lyra "La Gemela de Día"**: Una de las hermanas gemelas dimensionales
- **Nyx "La Gemela de Noche"**: La otra hermana gemela dimensional
- **Reflejos Independientes**: Los reflejos de los PJ que actúan por su cuenta
- **Clones de Espejo**: Versiones de los PJ que emergen de los espejos
- **Guardianes del Espejo**: Criaturas que protegen los portales dimensionales

### **Puntos de Interés**
- **El Salón de los Espejos**: Donde los PJ descubren los espejos mágicos
- **El Laberinto de Reflejos**: Pasillos donde los espejos muestran acciones diferentes
- **La Cámara de Colaboración**: Donde los PJ deben resolver puzzles con sus reflejos
- **El Santuario de las Gemelas**: Donde se enfrentan a Lyra y Nyx en combate final
- **Los Portales de Clonación**: Espejos que se convierten en portales para invocar clones

### **Mecánicas de Juego**
- **Dimensión Espejo**: Las gemelas pueden intercambiar posiciones instantáneamente
- **Clonación de Enemigos**: Pueden crear clones de los PJ que luchan contra ellos
- **Daño Compartido**: El daño recibido por los clones se comparte con el original (50%)
- **Refugio Dimensional**: Pueden refugiarse en la dimensión espejo, volviéndose inmunes
- **Estado de Acceso:** 🔒 **Bloqueada** - Accesible desde el inicio con precaución

### **Efectos Específicos de Las Gemelas del Espejo**
- **Intercambio de Posición**: Lyra y Nyx intercambian posiciones instantáneamente
- **Rayo Cegador**: Lanza un rayo de luz que ciega a un enemigo por 1 turno
- **Clon de Enemigo**: Crea un clon de un enemigo que lucha por las gemelas
- **Refugio Espejo**: Ambas se refugian en la dimensión espejo, inmunes a ataques
- **Ataque Dual**: Ambas atacan simultáneamente desde dimensiones opuestas
- **Clon Masivo**: Crea clones de 1d3 enemigos diferentes
- **Daño Compartido**: Todo el daño recibido por los clones se comparte con el original
- **Reflejos Múltiples**: Crea múltiples reflejos que confunden sobre cuál es real
- **Dimensión Invertida**: Invierte completamente la realidad en 60 pies
- **Colapso Dimensional**: Colapsa la dimensión espejo, atrapando enemigos

### **Personalidad Misteriosa**
No se sabe realmente si eran dos personas originalmente o si una surgió de la dimensión espejo. Ambas afirman ser la original y la otra el reflejo, creando una dinámica psicológica compleja donde la identidad y la realidad se confunden.

### **Acciones Legendarias**
Ver [00_Cronofagos_Detallado.md](../02_Guia_DM/04_Cronofagos_Detallado/00_Cronofagos_Detallado.md) para la lista completa de acciones legendarias escalables por nivel.

---

## ❄️ **Glacialis (Ymir el Eterno Invierno - Reclutador Interplanar)**

**Ubicación**: Región del extremo norte, glaciares eternos

**🎭 LUGARTENIENTE ACTIVO** - Participa en "La Ascensión del Cónclave"

**⚠️ FUNCIÓN CRÍTICA:** Ymir es el **Reclutador Interplanar** de los Cronófagos. Mantiene una red de portales que conecta Chronosia con múltiples planos (Plano Elemental del Agua, Plano Elemental del Aire, Plano Astral, Plano Material, y otros), permitiendo el reclutamiento de criaturas y el transporte de suministros interplanares. Glacialis funciona como el **centro de distribución interplanar** de los Cronófagos.

### **Descripción Geográfica**
Un reino de hielo eterno en el extremo norte de Chronosia, donde Ymir el Eterno Invierno domina con su control espacial a través del hielo. Los glaciares se extienden hasta donde alcanza la vista, y las auroras boreales temporales iluminan el cielo nocturno. El frío es tan intenso que el espacio mismo se cristaliza en patrones de hielo que existen en múltiples planos simultáneamente.

**🌌 Red de Portales Interplanares:**
Glacialis alberga múltiples portales de hielo que se abren constantemente, conectando con diferentes planos del multiverso. Estos portales son visibles como círculos de hielo cristalino que brillan con luz azulada, y a través de ellos fluyen criaturas, recursos y suministros desde otros planos. La región está constantemente activa con el tráfico interplanar, convirtiéndola en el **centro de distribución interplanar** de los Cronófagos.

**Planos Conectados:**
- **Plano Elemental del Agua**: Portales principales para elementales acuáticos
- **Plano Elemental del Aire**: Portales secundarios para criaturas aéreas
- **Plano Astral**: Conexiones para viaje rápido entre planos
- **Plano Material (Faerûn)**: Portales menores para infiltración
- **Otros Planos**: Conexiones variables según necesidades de los Cronófagos

**🌌 Red de Portales Interplanares:**
Glacialis alberga múltiples portales de hielo que se abren constantemente, conectando con diferentes planos del multiverso. Estos portales son visibles como círculos de hielo cristalino que brillan con luz azulada, y a través de ellos fluyen criaturas, recursos y suministros desde otros planos. La región está constantemente activa con el tráfico interplanar, convirtiéndola en un punto estratégico crucial para los Cronófagos.

### **Condiciones Especiales**
- El frío intenso congela el espacio y crea portales de hielo
- Los cristales de hielo existen en múltiples dimensiones
- Las distancias se comprimen y expanden a través del hielo
- El aliento de Ymir puede congelar dimensiones enteras

### **Habitantes**
- **Ymir el Eterno Invierno**: Gigante de hielo cristalino de 20 pies de altura
- **Elementales de Agua Dimensionales**: Criaturas del Plano Elemental del Agua
- **Guardianes de Hielo**: Que protegen los portales helados
- **Navegantes del Frío**: Que pueden moverse entre dimensiones a través del hielo

### **Puntos de Interés**
- **La Fortaleza de Hielo**: Residencia principal de Ymir y centro de operaciones del reclutamiento interplanar. Aquí se coordinan las operaciones de reclutamiento, transporte de suministros y expansión multidimensional de los Cronófagos.
- **La Red de Portales**: Múltiples portales de hielo que conectan con diferentes planos (Agua, Aire, Astral, Material, y otros). Los portales son visibles como círculos de hielo cristalino que brillan con luz azulada.
- **El Centro de Distribución**: Donde se almacenan y distribuyen suministros interplanares. Este es el corazón logístico de las operaciones de los Cronófagos en múltiples planos.
- **Los Campos de Cristal**: Donde el espacio se ha cristalizado en patrones de hielo que existen en múltiples planos simultáneamente.
- **El Laberinto Helado**: Donde las distancias cambian con el hielo, creando pasadizos dimensionales que conectan diferentes ubicaciones.

### **Mecánicas de Juego**
- **Compresión Espacial**: Reduce las distancias en 60 pies mediante hielo
- **Expansión Dimensional**: Aumenta las distancias en 90 pies
- **Portales de Hielo Interplanares**: Conectan con múltiples planos (Agua, Aire, Astral, Material, y otros). Estos portales permiten:
  - Reclutamiento de criaturas de otros planos
  - Transporte de suministros interplanares
  - Establecimiento de bases de operaciones en otros planos
  - Comunicación entre Chronosia y operaciones en otros planos
- **Reclutamiento Activo**: Criaturas y suministros llegan constantemente desde otros planos. La red de portales es esencial para mantener las operaciones de los Cronófagos.
- **Congelación Espacial**: El espacio se congela permanentemente
- **Función Estratégica**: La muerte de Ymir interrumpe significativamente el flujo de suministros y reclutamiento de los Cronófagos, debilitando sus operaciones en múltiples planos.
- **Estado de Acceso:** 🔓 **Desbloqueada** - Accesible desde el inicio

### **Efectos Específicos de Ymir el Eterno Invierno**
- **Aliento de Hielo**: Exhala un cono de hielo de 30 pies que reduce velocidad
- **Portal de Hielo**: Crea un portal al Plano Elemental del Agua
- **Cristalización**: Convierte el aire en cristales de hielo
- **Compresión Espacial**: Comprime el espacio en 60 pies
- **Portal de Elementales**: Trae 1d4 elementales de agua
- **Expansión Dimensional**: Expande el espacio en 90 pies
- **Congelación Masiva**: Congela toda el área en 120 pies
- **Portal Cósmico**: Crea un portal masivo que inunda la región

### **Acciones Legendarias**
Ver [00_Cronofagos_Detallado.md](../02_Guia_DM/04_Cronofagos_Detallado/00_Cronofagos_Detallado.md) para la lista completa de acciones legendarias escalables por nivel.

---

## 🎯 **ESTRATEGIAS DE COMBATE**

### **Contra Edward Teach "Barbanegra" (Principal - Independiente)**
- **CUIDADO:** Combina poderes dimensionales Y temporales después de asesinar a Thyra
- Prepararse para combinaciones devastadoras de portales y estasis
- Usar objetos que resistan tanto el tiempo como el espacio
- **ES UN PUNTO CIEGO PARA VARRAK:** No se puede predecir

### **Contra Dimensionalis la Fracturada (Principal - Independiente)**
- Prepararse para ser separados en habitaciones dimensionales
- Tener estrategias individuales para combate en solitario
- Usar objetos que resistan la compresión espacial
- Coordinar ataques antes de ser separados

### **Contra Ignis el Devorador Espacial (Principal - Líder Dimensional)**
- Resistir el calor intenso y las distorsiones espaciales
- Prepararse para oleadas constantes de crías de dragón y elementales
- Usar objetos resistentes al fuego dimensional
- Coordinar ataques durante las fusiones dimensionales

### **Contra Matrona Veldrisza (Principal - Bando Dimensional)**
- Evitar las redes espaciales y telarañas mágicas
- Usar objetos que resistan la magia drow
- Prepararse para enfrentar ejércitos drow invocados
- Interrumpir los portales a Menzoberranzan

### **Contra Las Gemelas del Espejo (Principales - Neutrales)**
- Colaborar con los reflejos para resolver puzzles
- Prepararse para enfrentar clones de sí mismos
- **CUIDADO:** El daño a los clones se comparte con el original
- Mantener el equilibrio entre ambas gemelas para evitar el caos

### **Contra Ymir el Eterno Invierno (Menor/Opcional)**
- Resistir el frío extremo y las distorsiones espaciales
- Usar objetos resistentes al hielo dimensional
- Aprovechar los portales de hielo para ventajas tácticas
- Interrumpir la invocación de elementales de agua

---

## ⏳ **EL RELOJ DEL RITUAL Y ESTAS REGIONES**

El coste de explorar cada región dimensional no es un "desbalance", sino el **tiempo**: cada arco a fondo avanza el Reloj del Ritual (la ascensión de Aethernus). Ver [Motor de Campaña](../02_Guia_DM/10_Motor_de_Campana_Reloj_y_Puertas.md).

- **Frena el Reloj:** derrotar a un lugarteniente **crítico** (aquí, **Ignis**, líder dimensional, o **Teach**).
- **Puertas que abre el ala dimensional:** derrotar a **Ignis** abre el ala profunda (Dimensionalis, Veldrisza); derrotar a **Teach** abre la Tela de Sombras de Veldrisza.

**NOTA ESPECIAL - Edward Teach:** Combina sus poderes piratas con los de estasis robados a Thyra (amenaza híbrida). Cada lugarteniente que los PJ no alcanzan, Teach lo absorbe y crece → su poder en la Torre depende de a cuántos llegó él antes que ellos.

---

*Estas regiones dimensionales ofrecen desafíos únicos que requieren creatividad y adaptación por parte de los jugadores. Cada una presenta mecánicas diferentes que mantendrán la campaña fresca y emocionante, mientras el Reloj del Ritual avanza al fondo.*
