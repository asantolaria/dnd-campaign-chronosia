# 📊 Tablas de Tracking de la Campaña
## *Hojas de Seguimiento para el DM*

---

> **⚠️ PROPÓSITO DE ESTE DOCUMENTO:**
> Este documento contiene **TODAS las tablas de tracking** necesarias para gestionar el progreso de la campaña Chronosia.
> 
> **Copia y actualiza estas tablas después de cada sesión** para mantener un registro completo del estado de la campaña.

---

## 📋 **ÍNDICE DE TABLAS**

1. [Sistema de Puntos de la Ascensión del Cónclave](#1-sistema-de-puntos-de-la-ascensión-del-cónclave)
2. [Progresión del Ritual de Manshoon](#2-progresión-del-ritual-de-manshoon)
3. [Balance Temporal/Dimensional](#3-balance-temporal-dimensional)
4. [Desbloqueo de Regiones](#4-desbloqueo-de-regiones)
5. [Progresión de Niveles de los PJ](#5-progresión-de-niveles-de-los-pj)
6. [Contador de Arena de Serapis](#6-contador-de-arena-de-serapis)
7. [Estado de Artefactos](#7-estado-de-artefactos)
8. [Estado de Lugartenientes](#8-estado-de-lugartenientes)
9. [Resolución Final y Estado para el Clímax](#9-resolución-final-y-estado-para-el-clímax)
10. [Resumen General de Sesión](#10-resumen-general-de-sesión)

---

## 1. **SISTEMA DE PUNTOS DE LA ASCENSIÓN DEL CÓNCLAVE**

### **📊 Tabla de Puntuación de Lugartenientes**

| **Lugarteniente** | **Bando** | **Pts Iniciales (Ses. 6)** | **Sesión 7** | **Sesión 8** | **Sesión 9** | **Sesión 10** | **Sesión 11** | **Sesión 12** | **TOTAL** | **Estado** |
|-------------------|-----------|---------------------------|--------------|--------------|--------------|---------------|---------------|---------------|-----------|------------|
| **Lord Vorthak** | Temporal | 0 | | | | | | | | |
| **Serapis** | Temporal | 0 | | | | | | | | |
| **Varrak** | Neutral → ? | 0 | | | | | **+3 o +5** | | | |
| **Ignis** | Dimensional | 0 | | | | | | | | |
| **Matrona Veldrisza** | Dimensional | 0 | | | | | | | | |
| **Edward Teach** | Independiente | **+5** (Thyra) | | | | | | | | |
| **Dimensionalis** | Independiente | 0 | | | | | | | | |
| **Las Gemelas** | Neutral → ? | 0 | | | | | **+2** | | | |
| **Medusa** | Menor | 0 | | | | | | | | |
| **Tempus** | Menor | 0 | | | | | | | | |
| **Ymir** | Menor | 0 | | | | | | | | |

### **📝 Notas de Tracking:**
- **Umbral de Ascensión:** 15 puntos (puede ocurrir desde Sesión 15)
- **Muerte Permanente:** -15 puntos (sale de la competencia)
- **Edward Teach Ventaja Inicial:** Comienza con +5 pts por asesinar a Thyra
- **Sesión 11 es CRÍTICA:** Varrak y Las Gemelas eligen bando

### **📋 Registro de Eventos por Sesión:**

| **Sesión** | **Fecha** | **Evento** | **Lugarteniente Afectado** | **Puntos** | **Notas** |
|------------|-----------|------------|----------------------------|------------|-----------|
| 6 | | Asesinato de Thyra | Edward Teach | +5 | Inicio del sistema |
| 7 | | | | | |
| 8 | | | | | |
| 9 | | | | | |
| 10 | | | | | |
| 11 | | Elección de Varrak | Varrak | +3 o +5 | Según decisión PJ |
| 11 | | Elección de Gemelas | Las Gemelas | +2 | Según bando ganador |
| 12 | | | | | |

---

## 2. **PROGRESIÓN DEL RITUAL DE MANSHOON**

### **⏰ Tabla de Progresión del Ritual**

| **Sesión** | **Fecha** | **Progresión** | **Estado** | **Eventos Clave** | **Notas** |
|------------|-----------|----------------|------------|------------------|-----------|
| **1-3** | | 85% | Inicio | Manshoon recluido | PJ no saben |
| **4-5** | | 86-87% | Avanzando | Robos de artefactos | Sin conocimiento |
| **6-7** | | 88-89% | Avanzando | Guerra espontánea | Sin conocimiento |
| **8** | | 90% | Avanzando | Primera sospecha + **Contador del Ritual creado** | PJ sospechan, Contador visible |
| **9** | | 91% | Avanzando | Exploración | Contador muestra 91% |
| **10** | | 92% | ⚠️ Urgencia | Revelación del ritual | PJ descubren |
| **11** | | 93-94% | ⚠️ Urgencia | Varrak elige bando | Urgencia creciente |
| **12** | | 95-97% | ⚠️⚠️ Crítico | Ritual al 92% revelado | Urgencia máxima |
| **13** | | 98% | ⚠️⚠️⚠️ Crítico | Llamada de los dioses | NO QUEDA TIEMPO |
| **14** | | 98-99% | ⚠️⚠️⚠️ Crítico | Asalto a la Torre | Durante combate |
| **15** | | 98-100% | ⚠️⚠️⚠️ Crítico | Combate final | Depende de timing |

### **📝 Notas de Tracking:**
- **Progresión Base:** +1% cada 2 sesiones (aproximadamente)
- **Aceleración:** Puede avanzar más rápido si los PJ se demoran
- **Punto Sin Retorno:** 98% (Sesión 13) - Los dioses llaman
- **Final D (Mal Final):** Si llega al 100% antes de que los PJ lleguen
- **⏰ Contador del Ritual:** Creado en Sesión 8-9 por Anacronistas y La Resistencia. Muestra el progreso del ritual de forma visible en Cronópolis. Ver [05_Encuentro_Anacronistas_Relojeros.md](../../04_Aventuras/Fase_2_Sandbox_Inicial/05_Encuentro_Anacronistas_Relojeros.md)

### **🎲 Modificadores de Progresión:**
- **PJ se demoran:** +1% adicional por sesión de demora
- **PJ actúan rápido:** Progresión normal
- **Eventos especiales:** Pueden acelerar o ralentizar

---

## 3. **BALANCE TEMPORAL/DIMENSIONAL**

### **⚖️ Tabla de Balance**

| **Sesión** | **Fecha** | **Temporales Derrotados** | **Dimensionales Derrotados** | **Diferencia** | **Estado** | **Efectos Activos** |
|------------|-----------|---------------------------|------------------------------|----------------|------------|---------------------|
| **1** | | 0 | 0 | 0 | ✅ Equilibrio | Ninguno |
| **2** | | 0 | 0 | 0 | ✅ Equilibrio | Ninguno |
| **3** | | 0 | 0 | 0 | ✅ Equilibrio | Ninguno |
| **4-5** | | 0 | 0 | 0 | ✅ Equilibrio | Ninguno |
| **6** | | 0 | 0 | 0 | ⚠️ Desequilibrio Inicial | 3T vs 5D (post-Thyra) |
| **7** | | | | | | |
| **8** | | | | | | |
| **9** | | | | | | |
| **10** | | | | | | |
| **11** | | | | | | |
| **12** | | | | | | |

### **📊 Estado Inicial (Sesión 6):**
- **Temporales Activos:** 3 (Serapis, Varrak, Vorthak)
- **Dimensionales Activos:** 5 (Teach, Dimensionalis, Ignis, Veldrisza, Gemelas)
- **Diferencia:** -2 (favorable a dimensionales)

### **📝 Niveles de Desbalance:**

| **Diferencia** | **Nivel** | **Efectos por Sesión** | **Descripción** |
|----------------|-----------|------------------------|-----------------|
| **0-1** | ✅ Equilibrio | Ninguno | Todo normal |
| **2** | ⚠️ Desbalance Menor | 1d4 efectos | Advertencias leves |
| **3** | ⚠️ Desbalance Moderado | 1d6 efectos | Efectos notables |
| **4** | ⚠️⚠️ Desbalance Crítico | 1d8 efectos | Efectos severos |
| **5+** | ⚠️⚠️⚠️ Catástrofe Inminente | 1d10 efectos | Punto sin retorno |
| **6+** | 💀 Catástrofe Cósmica | Apocalipsis | FINAL MALO |

### **⚠️ ACCIÓN ESPECIAL: SACRIFICIO DE VARRAK**

**Si el desbalance alcanza 4+ (Desbalance Crítico):**

Varrak del Horizonte, habiendo visto millones de futuros posibles, tomará la decisión de quitarse la vida para equilibrar los poderes y evitar la catástrofe cósmica.

**Condiciones:**
- Diferencia de 4+ entre temporales y dimensionales derrotados
- Varrak debe estar vivo y activo
- Varrak probablemente es aliado de los PJ (muy probable según desarrollo)

**Efecto Inmediato:**
- Varrak se quita la vida voluntariamente
- **Balance se restaura:** -1 temporal derrotado (Varrak)
- **Diferencia se reduce:** El desbalance crítico se mitiga significativamente
- **Región se restaura:** El Abismo de los Posibles vuelve a la normalidad
- **Poder temporal liberado:** Se disipa suavemente, estabilizando Chronosia

**Impacto:**
- ✅ Balance restaurado (diferencia se reduce en 1)
- ⚠️ Varrak muere (los PJ pierden un aliado poderoso)
- ⚠️ Momento emocional dramático
- ✅ Estabilización de Chronosia

**Nota:** Los PJ pueden intentar mantener el balance para evitar este sacrificio. Si actúan rápidamente para equilibrar antes de llegar a crítico, Varrak no necesita sacrificarse.

### **📋 Registro de Derrotas:**

| **Sesión** | **Fecha** | **Lugarteniente Derrotado** | **Tipo** | **Temporales** | **Dimensionales** | **Nueva Diferencia** | **Notas** |
|------------|-----------|------------------------------|----------|----------------|-------------------|----------------------|-----------|
| 1 | | Kaelthas | Dimensional Menor | 0 | 1 | -1 | |
| 6 | | Thyra | Temporal Caído | 0 | 0 | -2 (ya muerta) | Asesinada por Teach |
| | | Varrak (Sacrificio) | Temporal Principal | 1 | 0 | -1 | ⚠️ **Sacrificio voluntario por desbalance crítico** |
| | | | | | | | |
| | | | | | | | |

---

## 4. **DESBLOQUEO DE REGIONES**

### **🗺️ Tabla de Desbloqueo de Regiones**

| **Región** | **Lugarteniente** | **Tipo** | **Nivel** | **Sesión Desbloqueada** | **Fecha** | **Estado** | **Explorada** |
|-----------|------------------|----------|-----------|-------------------------|-----------|------------|---------------|
| **Waterdeep** | Kaelthas (Caído) | Material Plane | 3 | Sesión 1 | | ✅ Desbloqueada | ✅ |
| **Cronópolis** | — | Centro Seguro | — | Sesión 2 | | ✅ Desbloqueada | ✅ |
| **Glacialis** | Ymir | Dimensional Menor | 5-7 | Sesión 3 | | ✅ Desbloqueada | |
| **La Espiral del Reloj de Arena** | Serapis | Temporal Principal | 5-7 | Sesión 3 | | ✅ Desbloqueada | |
| **Valle de la Aguja Suspendida** | Thyra (Caído) | Temporal Caído | 6-8 | Sesión 3 | | ⚰️ Bloqueada (Thyra muerta) | |
| **Archipiélago de Barbanegra** | Edward Teach | Dimensional Principal | 11-13 | Sesión 4-5 | | ✅ Desbloqueada | |
| **El Abismo de los Posibles** | Varrak | Temporal Principal | 7-9 | Sesión 6 | | ✅ Desbloqueada | |
| **Volcanis** | Ignis | Dimensional Principal | 13-15 | Sesión 6 | | ✅ Desbloqueada | |
| **La Mansión de la Sed Eterna** | Vorthak | Temporal Principal | 9-11 | Sesión 6 | | ✅ Desbloqueada | |
| **Las Llanuras de la Compresión** | Dimensionalis | Dimensional Principal | 8-10 | Sesión 8 | | ✅ Desbloqueada | |
| **El Jardín de los Tiempos Gemelos** | Las Gemelas | Dimensional Principal | 8-10 | Sesión 8 | | ✅ Desbloqueada | |
| **El Desierto Espejado** | Tempus | Temporal Menor | 7-9 | Sesión 9 | | ✅ Desbloqueada | |
| **Tela de Sombras** | Veldrisza | Dimensional Principal | 10-12 | Sesión 10 | | ✅ Desbloqueada | |
| **El Jardín Pétreo** | Medusa | Temporal Menor | 6-8 | Sesión 11 | | ✅ Desbloqueada | |
| **Torre de la Eternidad** | Manshoon | Final | 13-15 | Sesión 13 | | ✅ Desbloqueada | |

### **📝 Notas de Tracking:**
- **Total de Regiones:** 14 (12 con lugartenientes + 1 Material Plane + 1 centro seguro)
- **Regiones Desbloqueadas:** Marca con ✅ cuando se desbloquea
- **Regiones Exploradas:** Marca con ✅ cuando los PJ la visitan
- **Regiones Bloqueadas:** Marca con ⚰️ si el lugarteniente muere y la región se bloquea

---

## 5. **PROGRESIÓN DE NIVELES DE LOS PJ**

### **📈 Tabla de Progresión de Niveles**

| **Sesión** | **Fecha** | **Nivel Inicial** | **Nivel Final** | **Método de Subida** | **Notas** |
|------------|-----------|------------------|-----------------|----------------------|-----------|
| **1** | | 3 | 3 | Inicio de campaña | |
| **2** | | 3 | 4 | Investigación completa | |
| **3** | | 4 | 5 | Primer lugarteniente menor | |
| **4-5** | | 5 | 6 | Eventos de robos (XP narrativo) | |
| **6** | | 6 | 7 | Reacción Espontánea | |
| **7** | | 7 | 8 | Derrotar 1-2 lugartenientes | |
| **8** | | 8 | 9 | Derrotar 1-2 lugartenientes | |
| **9** | | 9 | 10 | Derrotar 1 lugarteniente principal | |
| **10** | | 10 | 11 | Revelaciones + 1 lugarteniente | |
| **11** | | 11 | 12 | Evento de Varrak + 1 lugarteniente | |
| **12** | | 12 | 13 | Derrotar 1-2 lugartenientes | |
| **13** | | 13 | 13 | Preparación para clímax | |
| **14** | | 13 | 14 | Atravesar Torre (Niveles 1-4) | |
| **15** | | 14 | 15 | Combate final (Nivel 5) | |

### **📝 Tracking Individual de PJ:**

| **PJ** | **Nivel Actual** | **Última Subida** | **Próxima Subida** | **Notas** |
|--------|------------------|-------------------|-------------------|-----------|
| | | | | |
| | | | | |
| | | | | |
| | | | | |
| | | | | |
| | | | | |

---

## 6. **CONTADOR DE ARENA DE SERAPIS**

### **⏰ Tabla de Tracking del Contador de Arena**

**Región:** La Espiral del Reloj de Arena  
**Lugarteniente:** Serapis el Retroceso

| **Reinicio** | **Puntos Iniciales** | **Acciones que Bajan** | **Acciones que Suben** | **Puntos Finales** | **Resultado** |
|--------------|---------------------|------------------------|----------------------|-------------------|---------------|
| **1** | 20 | | | | |
| **2** | 20 | | | | |
| **3** | 20 | | | | |
| **4** | 20 | | | | ⚠️ Bucle Eterno |

### **📉 Acciones que BAJAN la Arena (Registro):**

| **Turno/Hora** | **Acción** | **Puntos Perdidos** | **Puntos Restantes** | **Notas** |
|----------------|------------|---------------------|----------------------|-----------|
| | Combate (cada turno) | -1 | | |
| | Hechizo nivel 3+ | -2 | | |
| | Daño a Serapis (25+ HP) | -1 | | |
| | Descanso corto | -3 | | |
| | Descanso largo | -5 | | |
| | Muerte de PJ | -2 | | |
| | Hora de oscuridad | -1 | | |

### **📈 Acciones que SUBEN la Arena (Registro):**

| **Turno/Hora** | **Acción** | **Puntos Ganados** | **Puntos Totales** | **Notas** |
|----------------|------------|-------------------|-------------------|-----------|
| | Destruir Cristal de Arena | +2 | | |
| | Resolver puzzle temporal | +3 | | |
| | Ayudar aldeanos (1 hora) | +1 | | |
| | Sacrificar objeto mágico | +5 | | |
| | Derrotar enemigos sin combate | +1 | | |
| | Ritual temporal con aldeanos | +4 | | |

### **📝 Notas de Tracking:**
- **Límite de Reinicios:** 3 veces máximo
- **Reinicio 1:** Aprendizaje de mecánica
- **Reinicio 2:** Optimización de estrategia
- **Reinicio 3:** Última oportunidad
- **Reinicio 4:** Bucle eterno (deben ser rescatados)

---

## 7. **ESTADO DE ARTEFACTOS**

### **💎 Tabla de Estado de Artefactos**

| **Artefacto** | **Ubicación Original** | **Estado Actual** | **Poseedor Actual** | **Sesión** | **Fecha** | **Notas** |
|---------------|----------------------|-------------------|---------------------|------------|-----------|-----------|
| **Cronómetro de Realidades** | Cronópolis (Anacronistas) | ⚠️ Robado | Edward Teach | Sesión 4 | | Componente del Talismán |
| **Perla del Vacío Primordial** | Abysara (Oceánicos de las Profundidades) | ⚠️ Robado | Edward Teach | Sesión 4-5 | | Componente del Talismán |
| **Talismán de Interceptación Divina** | — | ✅ Creado | Edward Teach | Sesión 5 | | ⚠️ SIEMPRE se crea (evento fijo) |
| **Rotor Infinito** | Fragmentado | 🔄 En construcción | Ingenieros | Post-campaña | | Núcleo + Carcasa |
| **Espejo de Realidad Fragmentada** | — | ❓ Desconocido | — | — | | Pendiente |
| **Arena del Tiempo Estabilizada** | — | ❓ Desconocido | — | — | | Pendiente |
| **Excavadora Dimensional** | — | ❓ Desconocido | — | — | | Pendiente |
| **Visión de Varrak** | — | ❓ Desconocido | — | — | | Pendiente |

### **📝 Estados:**
- **✅ En posesión:** Artefacto está con su dueño legítimo
- **⚠️ Robado:** Artefacto fue robado
- **🔄 En construcción:** Artefacto está siendo creado/ensamblado
- **❓ Desconocido:** Estado no determinado aún
- **💀 Destruido:** Artefacto fue destruido

### **📋 Cronología de Artefactos:**

| **Sesión** | **Fecha** | **Evento** | **Artefacto** | **Cambio de Estado** |
|------------|-----------|-----------|---------------|----------------------|
| 2 | | Los PJ ven el Cronómetro | Cronómetro | ✅ En Cronópolis |
| 4 | | Robo del Cronómetro | Cronómetro | ⚠️ Robado por Teach |
| 4-5 | | Robo de la Perla | Perla | ⚠️ Robado por Teach |
| 5 | | Creación del Talismán | Talismán | ✅ Creado (SIEMPRE - evento fijo) |

---

## 8. **ESTADO DE LUGARTENIENTES**

### **👑 Tabla de Estado de Lugartenientes**

| **Lugarteniente** | **Tipo**              | **Estado** | **Sesión de Cambio** | **Ubicación**             | **Notas**                     |
| ----------------- | --------------------- | ---------- | -------------------- | ------------------------- | ----------------------------- |
| **Kaelthas**      | Temporal Menor        | ⚰️ Muerto  | Sesión 1             | Waterdeep                 | Asesinado por PJ              |
| **Thyra**         | Temporal Caído        | ⚰️ Muerto  | Sesión 5-6           | Valle de la Aguja         | Asesinado por Teach           |
| **Serapis**       | Temporal Principal    | ✅ Activo   | —                    | Espiral del Reloj         | Subordinado de Vorthak        |
| **Varrak**        | Temporal Principal    | ✅ Activo   | —                    | Abismo de los Posibles    | Neutral hasta Sesión 11, ⚠️ puede sacrificarse si desbalance crítico |
| **Vorthak**       | Temporal Principal    | ✅ Activo   | —                    | Mansión de la Sed         | Líder Bando Temporal          |
| **Edward Teach**  | Dimensional Principal | ✅ Activo   | —                    | Archipiélago              | Independiente, tiene Talismán |
| **Dimensionalis** | Dimensional Principal | ✅ Activo   | —                    | Llanuras de Compresión    | Independiente                 |
| **Ignis**         | Dimensional Principal | ✅ Activo   | —                    | Volcanis                  | Líder Bando Dimensional       |
| **Veldrisza**     | Dimensional Principal | ✅ Activo   | —                    | Tela de Sombras           | Aliada de Ignis               |
| **Las Gemelas**   | Dimensional Principal | ✅ Activo   | —                    | Jardín de Tiempos Gemelos | Neutral hasta Sesión 11       |
| **Medusa**        | Temporal Menor        | ✅ Activo   | —                    | Jardín Pétreo             |                               |
| **Tempus**        | Temporal Menor        | ✅ Activo   | —                    | Desierto Espejado         |                               |
| **Ymir**          | Dimensional Menor     | ✅ Activo   | —                    | Glacialis                 |                               |
| **Manshoon**      | Líder Supremo         | ✅ Recluido | —                    | Torre de la Eternidad     | Ejecutando ritual             |

### **📝 Estados:**
- **✅ Activo:** Lugarteniente está vivo y activo
- **⚰️ Muerto:** Lugarteniente fue derrotado permanentemente
- **🤝 Aliado:** Lugarteniente se alió con los PJ
- **⚠️ Herido:** Lugarteniente está gravemente herido
- **🔒 Recluido:** Lugarteniente está aislado (Manshoon)

### **📋 Cambios de Estado:**

| **Sesión** | **Fecha** | **Lugarteniente** | **Cambio** | **Causa** | **Notas** |
|------------|-----------|-------------------|------------|-----------|-----------|
| 1 | | Kaelthas | ⚰️ Muerto | Derrotado por PJ | |
| 5-6 | | Thyra | ⚰️ Muerto | Asesinado por Teach | Poder robado |
| 11 | | Varrak | Cambio de bando | Decisión de PJ | +3 o +5 pts |
| 11 | | Las Gemelas | Cambio de bando | Según bando ganador | +2 pts |
| X | | Varrak | ⚰️ Muerto (Sacrificio) | Desbalance crítico (4+) | ⚠️ **Sacrificio voluntario para equilibrar poderes** |

---

## 9. **RESOLUCIÓN FINAL Y ESTADO PARA EL CLÍMAX**

### **📊 Tabla de Estado Final (Sesión 13)**

**⚠️ IMPORTANTE:** Completa esta tabla ANTES de la Sesión 13 para determinar el estado final del combate.

| **Factor** | **Valor** | **Notas** |
|------------|-----------|-----------|
| **Número de Supremos (15+ pts)** | _____ | Lista: _____ |
| **Número de Lugartenientes Vivos** | _____ | Lista: _____ |
| **Lugartenientes Aliados con PJ** | _____ | Lista: _____ |
| **Edward Teach: ¿Tiene Talismán?** | Sí / No | |
| **Edward Teach: ¿Está Vivo?** | Sí / No | |
| **Edward Teach: ¿Es Supremo?** | Sí / No | |
| **Balance Temporal/Dimensional** | _____ | Diferencia: _____ |

### **🎯 Matriz de Decisión del Combate Final:**

| **Supremos** | **Lugartenientes Vivos** | **Aliados PJ** | **Escenario Probable** | **Dificultad** |
|--------------|---------------------------|----------------|------------------------|----------------|
| **0** | **0-2** | **No** | Escenario 1: Solo Manshoon | ⭐⭐⭐ |
| **1-2** | **1-3** | **No** | Escenario 2: Manshoon + Supremos | ⭐⭐⭐⭐⭐ |
| **1-2** | **2-4** | **Sí** | Escenario 3: Guerra de Tres Bandos | ⭐⭐⭐⭐ |
| **0** | **1-3** | **Sí** | Escenario 4: PJ + Aliados vs Manshoon | ⭐⭐ |
| **3** | **3-5** | **No** | Escenario 2: Múltiples Supremos | ⭐⭐⭐⭐⭐ |

### **📋 Estado Detallado de Lugartenientes para el Clímax:**

| **Lugarteniente** | **Puntos** | **¿Es Supremo?** | **Estado** | **¿Aliado PJ?** | **¿En Torre?** | **Notas** |
|-------------------|------------|------------------|------------|-----------------|----------------|-----------|
| **Vorthak** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Serapis** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Varrak** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Ignis** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Veldrisza** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Edward Teach** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Dimensionalis** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Las Gemelas** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Medusa** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Tempus** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |
| **Ymir** | | Sí / No | Vivo / Muerto | Sí / No | Sí / No | |

### **📊 Impacto en los Finales:**

**Final Determinado:** _____ (A / B / C / D)

| **Final** | **Estado de Lugartenientes** | **Impacto en Dificultad** | **Modificadores** |
|-----------|------------------------------|---------------------------|-------------------|
| **A: Manshoon** | _____ | _____ | _____ |
| **B: Blackbeard** | _____ | _____ | _____ |
| **C: Carrera Divina** | _____ | _____ | _____ |
| **D: Demasiado Tarde** | _____ | _____ | _____ |

---

## 10. **RESUMEN GENERAL DE SESIÓN**

### **📋 Plantilla de Resumen de Sesión**

**Sesión #:** _____  
**Fecha:** _____  
**Nivel de PJ:** _____  
**Duración:** _____ horas

#### **Eventos Principales:**
- 
- 
- 

#### **Lugartenientes Enfrentados:**
- 
- 

#### **Regiones Exploradas:**
- 
- 

#### **Cambios en Puntos de Ascensión:**
- 
- 

#### **Progresión del Ritual:**
- **Antes:** _____%
- **Después:** _____%

#### **Balance Temporal/Dimensional:**
- **Temporales Derrotados:** _____
- **Dimensionales Derrotados:** _____
- **Diferencia:** _____
- **Estado:** _____

#### **Artefactos:**
- 
- 

#### **Decisiones Críticas de los PJ:**
- 
- 

#### **Notas para Próxima Sesión:**
- 
- 

---

## 📝 **INSTRUCCIONES DE USO**

### **Cómo Usar Estas Tablas:**

1. **Antes de la Primera Sesión:**
   - Copia todas las tablas a un documento separado (o usa este mismo)
   - Inicializa todos los valores base (Sesión 1-6)

2. **Después de Cada Sesión:**
   - Actualiza la tabla de Puntos de Ascensión
   - Actualiza el Progresión del Ritual
   - Actualiza el Balance Temporal/Dimensional
   - Marca regiones exploradas
   - Actualiza niveles de PJ
   - Registra cambios de estado de lugartenientes
   - Completa el Resumen General de Sesión

3. **Antes de la Sesión 13 (Clímax):**
   - **COMPLETA la Tabla de Resolución Final** (Sección 9)
   - Determina qué lugartenientes están vivos, quién es Supremo, quién es aliado
   - Calcula el escenario probable del combate final
   - Determina qué final ocurrirá según el estado

4. **Durante la Sesión:**
   - Si los PJ visitan la región de Serapis, usa la tabla del Contador de Arena
   - Si hay eventos aleatorios, actualiza puntos inmediatamente
   - Si derrotan a un lugarteniente, actualiza balance y estado

5. **Revisión Periódica:**
   - Cada 3 sesiones, revisa que todo esté actualizado
   - Verifica que el balance no esté demasiado desequilibrado
   - Asegúrate de que el ritual avanza apropiadamente

---

## 🔗 **REFERENCIAS**

- **[06_Eventos_Ascension_Conclave.md](../../02_Guia_DM/06_Eventos_Ascension_Conclave.md)** - Sistema completo de puntos
- **[07_Cronologia_Maestra_Campana.md](../../02_Guia_DM/07_Cronologia_Maestra_Campana.md)** - Timeline maestro
- **[10_Regiones_Temporales.md](../../03_Regiones/10_Regiones_Temporales.md)** - Sistema del Contador de Arena
- **[08_Geografia_y_Regiones_de_Chronosia.md](../../03_Regiones/08_Geografia_y_Regiones_de_Chronosia.md)** - Información de regiones

---

*Estas tablas son herramientas vivas. Actualízalas constantemente y mantén un registro detallado del progreso de tu campaña.* 📊✨

